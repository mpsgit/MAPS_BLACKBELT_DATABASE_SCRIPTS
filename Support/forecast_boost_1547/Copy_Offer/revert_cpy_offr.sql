           ...
           IF ln_job_typ = 1 THEN
              ...
              BEGIN
                 SELECT COUNT (sku_id)
                   ...
                   FROM (SELECT srce_sku.sku_id,
                                pa_maps.get_sls_cls_cd
                                   (ln_trgt_offr_perd_id,        -- target offer period id
                                    ln_trgt_mrkt_id,             -- target market id
                                    t_mrkt_sku.avlbl_perd_id,
                                    t_mrkt_sku.intrdctn_perd_id,
                                    t_mrkt_sku.demo_ofs_nr,
                                    t_mrkt_sku.demo_durtn_nr,
                                    t_mrkt_sku.new_durtn_nr,
                                    t_mrkt_sku.stus_perd_id,
                                    t_mrkt_sku.dspostn_perd_id,
                                    t_mrkt_sku.on_stus_perd_id
                                   ) sales_class_code_target
                           FROM (SELECT DISTINCT osl.sku_id
                                            FROM mrkt_veh mv,
                                                 mrkt_veh_perd mvp,
                                           ...
                                           WHERE mv.mrkt_id = ln_srce_mrkt_id
                                             ...
                                             AND osl.dltd_ind <> 'Y') srce_sku,
                                (SELECT *
                                   FROM mrkt_sku
                                  WHERE mrkt_id = ln_trgt_mrkt_id) t_mrkt_sku
                          WHERE srce_sku.sku_id = t_mrkt_sku.sku_id(+))
                  WHERE sales_class_code_target NOT IN (
                           --getting the forecast boost excluded sls_cls_cd for the target mrkt perd
                           SELECT DISTINCT sls_cls_cd
                                      FROM frcst_boost_xclusn
                                     WHERE mrkt_id = ln_trgt_mrkt_id
                                       AND ln_trgt_offr_perd_id
                                              BETWEEN strt_offr_perd_id
                                                  AND NVL (end_offr_perd_id, 99990399));
              EXCEPTION
                 WHEN OTHERS THEN
                    app_plsql_log.info
                         ...
                         FROM (SELECT srce_sku.sku_id,
                                      pa_maps.get_sls_cls_cd
                                         (l_tbl_trgt_offr_perd_id (i),
                                          -- target offer period id
                                          l_tbl_trgt_mrkt_id (i),  -- target market id
                                          t_mrkt_sku.avlbl_perd_id,
                                          t_mrkt_sku.intrdctn_perd_id,
                                          t_mrkt_sku.demo_ofs_nr,
                                          t_mrkt_sku.demo_durtn_nr,
                                 ...
                                 FROM (SELECT DISTINCT osl.sku_id
                                      ...
                                      (SELECT *
                                         FROM mrkt_sku
                                        WHERE mrkt_id = l_tbl_trgt_mrkt_id (i)) t_mrkt_sku
                                WHERE srce_sku.sku_id = t_mrkt_sku.sku_id(+))
                        WHERE sales_class_code_target NOT IN (
                                 --getting the forecast boost excluded sls_cls_cd for the target mrkt perd
                                 SELECT DISTINCT sls_cls_cd
                                            FROM frcst_boost_xclusn
                                           WHERE mrkt_id = l_tbl_trgt_mrkt_id (i)
                                             AND l_tbl_trgt_offr_perd_id (i)
                                                    BETWEEN strt_offr_perd_id
                                                        AND NVL (end_offr_perd_id,
                                                                 99990399
                                                                ));
...
APP_PLSQL_LOG.info('Start of P_ADD_ADD_REG_PRC_ITEMS');
   ...
   BEGIN
    ...
    if p_fb_ind = 'Y' then
        ...
        app_plsql_log.info(l_procedure_name||'Excluded sales classes '||l_fb_xclud_sls_cls_cd);
