create or replace PACKAGE BODY pa_trend_alloc AS
  ...
  FUNCTION get_ta_config(p_mrkt_id        IN ta_config.mrkt_id%TYPE,
    ...
    l_sls_typ_grp_nm VARCHAR2(128) := nvl(p_sls_typ_grp_nm,
                                          c_sls_typ_grp_nm_bi24 || ', ' ||
                                          c_sls_typ_grp_nm_dms || ', ' ||
                                          c_sls_typ_grp_nm_fc_dbt || ', ' ||
                                          c_sls_typ_grp_nm_fc_dms);
    l_parameter_list VARCHAR2(2048) := ' (p_mrkt_id: ' ||
                                       to_char(p_mrkt_id) || ', ' ||
                                       'p_sls_perd_id: ' ||
  ...
  BEGIN
    ...
    --
    BEGIN
      FOR i IN (SELECT sls_typ_grp_nm,
                       src_sls_typ_id,
                       x_src_sls_typ_id,
                       offst_lbl_id,
                       sls_typ_lbl_id,
  ...
  -- init_temp_config
  PROCEDURE init_temp_config(p_mrkt_id        IN mrkt_trnd_sls_perd_sls_typ.mrkt_id%TYPE,
    ...
    l_run_id         NUMBER := nvl(p_run_id,
                                   app_plsql_output.generate_new_run_id);
    l_user_id        VARCHAR2(35) := nvl(p_user_id, USER());
    l_module_name    VARCHAR2(30) := 'INIT_TEMP_CONFIG';
    l_parameter_list VARCHAR2(2048) := ' (p_mrkt_id: ' ||
                                       to_char(p_mrkt_id) || ', ' ||
                                       'p_campgn_perd_id: ' ||
    ...
    INSERT INTO temp_config
      ...
      (SELECT src_sls_typ_id,
         ...
         FROM TABLE(get_ta_config(p_mrkt_id        => p_mrkt_id,
                                  ...
                                  p_sls_typ_grp_nm => p_sls_typ_grp_nm,
                                  p_user_id        => l_user_id,
                                  p_run_id         => l_run_id)));
    COMMIT;
    app_plsql_log.set_context(l_user_id, g_package_name, l_run_id);
  ...
  BEGIN
    ...
    app_plsql_log.set_context(l_user_id, g_package_name, l_run_id);
    app_plsql_log.info(l_module_name || ' start' || l_parameter_list);
    -- load ALL VALUES into TEMP_CONFIG
    init_temp_config(p_mrkt_id        => p_mrkt_id,
                     p_campgn_perd_id => p_campgn_perd_id,
                     p_sls_typ_id     => p_sls_typ_id,
    ...
    BEGIN
      ...
      BEGIN
        SELECT MAX(trgt_sls_perd_id)
          ...
          FROM temp_config WHERE INSTR(c_sls_typ_grp_nm_bi24, sls_typ_grp_nm) > 0;
      EXCEPTION
        WHEN OTHERS THEN
      ...
      USING (SELECT p_campgn_perd_id AS campgn_perd_id,
                    ...
                    CASE
                      WHEN xm.int_lbl_desc = 'ON-SCHEDULE' THEN
                       mtspst.sct_cash_value_on
                      WHEN xm.int_lbl_desc = 'OFF-SCHEDULE' THEN
                       mtspst.sct_cash_value_off
                      ELSE
                       NULL
                    END AS cash_value,
  ...
  FUNCTION get_rule_attr(p_mrkt_id        IN dstrbtd_mrkt_sls.mrkt_id%TYPE,
                         ...
                         p_attr_nm        IN VARCHAR2) RETURN VARCHAR2 IS
    -- local variables
    l_rul_attr VARCHAR2(2048);
  BEGIN
    BEGIN
      SELECT CASE
               WHEN p_attr_nm = 'rul_id' THEN
                CAST(rul_id AS VARCHAR2(2048))
               WHEN p_attr_nm = 'rul_nm' THEN
                CAST(rul_nm AS VARCHAR2(2048))
               WHEN p_attr_nm = 'sku_list' THEN
                CAST(sku_list AS VARCHAR2(2048))
               WHEN p_attr_nm = 'prirty' THEN
                CAST(prirty AS VARCHAR2(2048))
               WHEN p_attr_nm = 'period_list' THEN
                CAST(period_list AS VARCHAR2(2048))
               WHEN p_attr_nm = 'r_factor' THEN
                CAST(nvl(r_factor, r_factor_manual) AS VARCHAR2(2048))
               WHEN p_attr_nm = 'cash_value' THEN
                CAST(cash_value AS VARCHAR2(2048))
               WHEN p_attr_nm = 'use_estimate' THEN
                CAST(use_estimate AS VARCHAR2(2048))
               ELSE
                NULL
             END
        INTO l_rul_attr
        FROM (SELECT rul_id,
               ...
               WHERE prirty IS NOT NULL
                 AND nvl(p_offst_lbl_id, -1) =
                     nvl(offst_lbl_id(+), nvl(p_offst_lbl_id, -1))
                 AND nvl(p_catgry_id, -1) =
                     nvl(catgry_id(+), nvl(p_catgry_id, -1))
                 AND nvl(p_sls_cls_cd, '-1') =
                     nvl(sls_cls_cd(+), nvl(p_sls_cls_cd, '-1'))
                 AND nvl(p_veh_id, -1) = nvl(veh_id(+), nvl(p_veh_id, -1))
                 AND nvl(p_perd_part, -1) =
                     nvl(perd_part(+), nvl(p_perd_part, -1))
                 AND instr(nvl(sku_list(+), nvl(p_sku_id, '-1')),
                           nvl(p_sku_id, '-1')) > 0)
      ...
      BEGIN
        SELECT greatest(MAX(dstrbtd_mrkt_sls.last_updt_ts),
          ...
          FROM dstrbtd_mrkt_sls,
               offr_prfl_prc_point,
               offr_sku_line,
               (SELECT * FROM temp_config WHERE INSTR(c_sls_typ_grp_nm_dms, sls_typ_grp_nm) > 0) tc_dms
    ...
    IF p_sls_typ_grp_nm = c_sls_typ_grp_nm_bi24 THEN
      BEGIN
        SELECT MAX(dly_bilng_trnd.last_updt_ts) last_updated_ts
          ...
          FROM dly_bilng_trnd,
               (SELECT * FROM temp_config WHERE INSTR(c_sls_typ_grp_nm_bi24, sls_typ_grp_nm) > 0) tc_bi24
  ...
  BEGIN
    ...
    BEGIN
      SELECT MAX(trgt_sls_perd_id)
        ...
        FROM temp_config
       WHERE instr(c_sls_typ_grp_nm_bi24, sls_typ_grp_nm) > 0;
    EXCEPTION
      WHEN OTHERS THEN
        app_plsql_log.set_context(l_user_id, g_package_name, l_run_id);
        app_plsql_log.info(l_module_name ||
                           ' warning: RECORD not found in GLOBAL TEMPORARY TABLE: temp_config where sls_typ_grp_nm IN (' ||
                           c_sls_typ_grp_nm_bi24 || ')' ||
                           l_parameter_list);
      ...
      IF l_tbl_bi24_perd.count > 0 THEN
        FOR i IN l_tbl_bi24_perd.first .. l_tbl_bi24_perd.last LOOP
          c_key := to_char(l_tbl_bi24_perd(i).sku_id) || '_' ||
                   ...
                   to_char(l_tbl_bi24_perd(i).promtn_clm_id) || '_' || l_tbl_bi24_perd(i)
                  .sls_cls_cd || '_' ||
          ...
          l_tbl_bi24_ind(c_key).promtn_clm_id := l_tbl_bi24_perd(i)
                                                 .promtn_clm_id;
          l_tbl_bi24_ind(c_key).sls_cls_cd := l_tbl_bi24_perd(i).sls_cls_cd;
          l_tbl_bi24_ind(c_key).offst_lbl_id := l_tbl_bi24_perd(i)
                                                .offst_lbl_id;
          l_tbl_bi24_ind(c_key).cash_value := l_tbl_bi24_perd(i).cash_value;
          l_tbl_bi24_ind(c_key).r_factor := l_tbl_bi24_perd(i).r_factor;
          l_tbl_bi24_ind(c_key).sls_typ_lbl_id := l_tbl_bi24_perd(i)
                                                  .sls_typ_lbl_id;
          l_tbl_bi24_ind(c_key).units := nvl(l_tbl_bi24_ind(c_key).units, 0) + l_tbl_bi24_perd(i)
                                        .units;
          l_tbl_bi24_ind(c_key).sales := nvl(l_tbl_bi24_ind(c_key).sales, 0) + l_tbl_bi24_perd(i)
                                        .sales;
        END LOOP;
  ...
  BEGIN
    ...
    init_temp_config(p_mrkt_id        => p_mrkt_id,
                     ...
                     p_user_id        => l_user_id,
                     p_run_id         => l_run_id);
    -- get trg_perd_id from TEMP_CONFIG
    BEGIN
      SELECT MAX(trgt_sls_perd_id)
        INTO l_trgt_sls_perd_id
        FROM temp_config
    ...
    EXCEPTION
      WHEN OTHERS THEN
        ...
        app_plsql_log.info(l_module_name ||
                           ' warning: RECORD not found in GLOBAL TEMPORARY TABLE: temp_config where sls_typ_grp_nm IN (' ||
                           c_sls_typ_grp_nm_bi24 || ')' || ' error code: ' ||
                           SQLCODE || ' error message: ' || SQLERRM ||
                           l_parameter_list);
        l_trgt_sls_perd_id := NULL;
    END;
    ...
    BEGIN
      SELECT SUM(dly_bilng_trnd.unit_qty)
        ...
        FROM dly_bilng_trnd,
             ...
             (SELECT *
                FROM temp_config
               WHERE instr(c_sls_typ_grp_nm_bi24, sls_typ_grp_nm) > 0) tc_bi24
       WHERE dly_bilng_trnd.mrkt_id = p_mrkt_id
         AND dly_bilng_trnd.trnd_sls_perd_id = tc_bi24.src_sls_perd_id
         AND dly_bilng_trnd.offr_perd_id = tc_bi24.src_offr_perd_id
      ...
      ELSE
        ...
        SELECT offr_sku_line.sku_id,
          ...
          FROM dstrbtd_mrkt_sls,
               ...
               (SELECT *
                  FROM temp_config
                 WHERE instr(c_sls_typ_grp_nm_dms, sls_typ_grp_nm) > 0) tc_dms
         WHERE dstrbtd_mrkt_sls.mrkt_id = p_mrkt_id
           AND dstrbtd_mrkt_sls.sls_perd_id = tc_dms.trgt_sls_perd_id
           AND dstrbtd_mrkt_sls.offr_perd_id = tc_dms.trgt_offr_perd_id
           AND dstrbtd_mrkt_sls.sls_typ_id = tc_dms.src_sls_typ_id
  ...
  BEGIN
    ...
    FOR p IN p_perd_to .. p_perd_from LOOP
      ...
      FOR i_prd IN (SELECT pa_maps_public.perd_plus(p_mrkt_id,
                                                    ...
                                                    (10000 * l_multplyr),
                                                    period) perd_id
                      FROM (SELECT DISTINCT TRIM(regexp_substr(col,
                                                               '[^,]+',
                                                               1,
                                                               LEVEL)) * CASE
                                              WHEN l_multplyr = 0 THEN
                                               0
                                              ELSE
                                               1
                                            END AS period
                              FROM (SELECT l_perd_list col FROM dual)
                            CONNECT BY LEVEL <=
                                       length(regexp_replace(col, '[^,]+')) + 1)
          ...
          IF l_tbl_hist_detail.count > 0 THEN
            FOR i IN l_tbl_hist_detail.first .. l_tbl_hist_detail.last LOOP
              ...
              l_tbl_hist_head(c_key).offst_lbl_id := l_tbl_hist_detail(i)
                                                     .offst_lbl_id;
              l_tbl_hist_head(c_key).units := nvl(l_tbl_hist_head(c_key)
                                                  .units,
                                                  0) + l_tbl_hist_detail(i)
                                             .units;
              l_tbl_hist_head(c_key).sales := nvl(l_tbl_hist_head(c_key)
                                                  .sales,
                                                  0) + l_tbl_hist_detail(i)
                                             .sales;
  ...
  BEGIN
    ...
    -- load ALL VALUES into TEMP_CONFIG
    init_temp_config(p_mrkt_id        => p_mrkt_id,
                     p_campgn_perd_id => p_sls_perd_id,
                     p_sls_typ_id     => p_sls_typ_id,
                     p_sls_typ_grp_nm => NULL,
                     p_user_id        => l_user_id,
                     p_run_id         => l_run_id);
  ...
  BEGIN
    ...
    BEGIN
      SELECT MAX(trgt_sls_perd_id), MAX(src_sls_typ_id)
        ...
        FROM temp_config
       WHERE instr(c_sls_typ_grp_nm_fc_dbt, sls_typ_grp_nm) > 0;
    EXCEPTION
      WHEN OTHERS THEN
        app_plsql_log.set_context(l_user_id, g_package_name, l_run_id);
        app_plsql_log.info(l_module_name ||
  ...
  BEGIN
    ...
    BEGIN
      SELECT offr_sku_line_id,
             ...
             round(SUM(nvl(unit_qty * nvl(r_factor, tc_fc_dms_r_factor), 0) *
                       sls_prc_amt / decode(nr_for_qty, 0, 1, nr_for_qty) *
        ...
        FROM (SELECT dstrbtd_mrkt_sls.offr_sku_line_id,
                ...
                FROM dstrbtd_mrkt_sls,
                     ...
                     (SELECT *
                        FROM temp_config
                       WHERE instr(c_sls_typ_grp_nm_fc_dms, sls_typ_grp_nm) > 0) tc_fc_dms
               WHERE dstrbtd_mrkt_sls.mrkt_id = p_mrkt_id
                 AND dstrbtd_mrkt_sls.sls_perd_id =
                     tc_fc_dms.src_sls_perd_id
                 AND dstrbtd_mrkt_sls.offr_perd_id =
                     tc_fc_dms.src_offr_perd_id
                 AND dstrbtd_mrkt_sls.sls_typ_id = tc_fc_dms.src_sls_typ_id
                 AND dstrbtd_mrkt_sls.offr_sku_line_id =
                     offr_sku_line.offr_sku_line_id
                 AND offr_sku_line.dltd_ind <> 'Y'
                 AND offr_sku_line.offr_prfl_prcpt_id =
  ...
  BEGIN
    ...
    IF l_tbl_bi24.count > 0 THEN
      ...
      FOR i IN (SELECT rul_nm,
        ...
        l_tbl_reproc_rule(c_key).units_bi24 := nvl(l_tbl_reproc_rule(c_key)
                                                   .units_bi24,
                                                   0) + i.units;
        l_tbl_reproc_rule(c_key).sales_bi24 := nvl(l_tbl_reproc_rule(c_key)
                                                   .sales_bi24,
                                                   0) + i.sales;
      END LOOP;
    END IF;
    ...
    IF l_tbl_reproc_trnd.count > 0 THEN
      FOR i IN l_tbl_reproc_trnd.first .. l_tbl_reproc_trnd.last LOOP
        ...
        l_tbl_reproc_rule(c_key).units_actual := nvl(l_tbl_reproc_rule(c_key)
                                                     .units_actual,
                                                     0) + l_tbl_reproc_trnd(i)
                                                .units;
        l_tbl_reproc_rule(c_key).sales_actual := nvl(l_tbl_reproc_rule(c_key)
                                                     .sales_actual,
                                                     0) + l_tbl_reproc_trnd(i)
                                                .sales;
      END LOOP;
    ...
    IF l_tbl_reproc_trnd_fsc_cd.count > 0 THEN
      FOR i IN l_tbl_reproc_trnd_fsc_cd.first .. l_tbl_reproc_trnd_fsc_cd.last LOOP
        ...
        l_tbl_reproc_rule(c_key).rul_nm := l_tbl_reproc_trnd_fsc_cd(i)
                                           .rul_nm;
        l_tbl_reproc_rule(c_key).units_actual := nvl(l_tbl_reproc_rule(c_key)
                                                     .units_actual,
                                                     0) + l_tbl_reproc_trnd_fsc_cd(i)
                                                .units;
        l_tbl_reproc_rule(c_key).sales_actual := nvl(l_tbl_reproc_rule(c_key)
                                                     .sales_actual,
                                                     0) + l_tbl_reproc_trnd_fsc_cd(i)
    ...
    IF l_tbl_reproc_trendsetter.count > 0 THEN
      FOR i IN l_tbl_reproc_trendsetter.first .. l_tbl_reproc_trendsetter.last LOOP
        ...
        l_tbl_reproc_rule(c_key).rul_nm := l_tbl_reproc_trendsetter(i)
                                           .rul_nm;
        -- BI24
        l_tbl_reproc_rule(c_key).units_bi24 := nvl(l_tbl_reproc_rule(c_key)
                                                   .units_bi24,
                                                   0) + l_tbl_reproc_trendsetter(i)
                                              .units_bi24;
        l_tbl_reproc_rule(c_key).sales_bi24 := nvl(l_tbl_reproc_rule(c_key)
                                                   .sales_bi24,
                                                   0) + l_tbl_reproc_trendsetter(i)
    ...
    IF l_tbl_reproc_rule.count > 0 THEN
      ...
      WHILE c_key IS NOT NULL LOOP
        PIPE ROW(obj_pa_trend_alloc_sgmnt_line(l_tbl_reproc_rule(c_key)
                                               .rul_nm,
                                               p_campgn_perd_id,
                                               p_bilng_day,
                                               NULL,
                                               round(l_tbl_reproc_rule(c_key)
                                                     .units_bi24),
                                               round(l_tbl_reproc_rule(c_key)
                                                     .sales_bi24),
                                               round(l_tbl_reproc_rule(c_key)
  ...
  BEGIN
    ...
    FOR i_prd IN (SELECT pa_maps_public.perd_plus(p_mrkt_id,
                    ...
                    FROM (SELECT LEVEL dummy
                         ...
                         (SELECT DISTINCT TRIM(regexp_substr(col,
                                                             '[^,]+',
                                                             1,
                                                             LEVEL)) * CASE
                                            WHEN l_multplyr = 0 THEN
                                        ...
                                        FROM (SELECT t_bi24.offst_lbl_id,
                                                      ...
                                                      CASE
                                                        WHEN t_bi24.units_sum = 0 THEN
                                                         t_actual.units / t_bi24.units_cnt
                                                        ELSE
                                                         t_actual.units * (t_bi24.units / t_bi24.units_sum)
                                                      END AS units_actual,
                                                      CASE
                                                        WHEN t_bi24.sales_sum = 0 THEN
                                                         t_actual.sales / t_bi24.units_cnt
                                                        ELSE
                                                         t_actual.sales * (t_bi24.sales / t_bi24.sales_sum)
                                                      END AS sales_actual
                                                 FROM (SELECT offst_lbl_id,
                                                              catgry_id,
                                                              sls_cls_cd,
  ...
  BEGIN
    ...
    SELECT offst_lbl_id,
      ...
      FROM (SELECT tc_fc_dms.offst_lbl_id,
              ...
              FROM dstrbtd_mrkt_sls,
                   ...
                   sku,
                   prfl,
                   (SELECT * FROM temp_config WHERE INSTR(c_sls_typ_grp_nm_fc_dms, sls_typ_grp_nm) > 0) tc_fc_dms
             WHERE dstrbtd_mrkt_sls.mrkt_id = p_mrkt_id
               AND dstrbtd_mrkt_sls.sls_perd_id = tc_fc_dms.src_sls_perd_id
               AND dstrbtd_mrkt_sls.offr_perd_id =
                   tc_fc_dms.src_offr_perd_id
               AND dstrbtd_mrkt_sls.sls_typ_id = tc_fc_dms.src_sls_typ_id
               AND dstrbtd_mrkt_sls.offr_sku_line_id =
    ...
    IF l_tbl_trendsetter.count > 0 THEN
      FOR i IN l_tbl_trendsetter.first .. l_tbl_trendsetter.last LOOP
        c_key := c_key + 1;
        l_tbl_current(c_key).offst_lbl_id := l_tbl_trendsetter(i)
                                             .offst_lbl_id;
    ...
    IF l_tbl_current.count > 0 THEN
      ...
      WHILE c_key IS NOT NULL LOOP
        PIPE ROW(obj_pa_trend_alloc_crrnt_line(l_tbl_current(c_key)
                                               .offst_lbl_id,
                                               l_tbl_current(c_key)
                                               .catgry_id,
                                               l_tbl_current(c_key)
  ...
  BEGIN
    ...
    BEGIN
      ...
      FOR p_rule IN c_p_rules LOOP
        ...
        IF p_stus = 0 THEN
          IF p_rule.prirty > 0 THEN
            IF p_rule.r_factor IS NULL
               AND p_rule.r_factor_manual IS NULL THEN
            ...
            ELSE
              ...
              VALUES
                (p_campgn_perd_id,
                 ...
                 p_rule.cash_value,
                 CASE WHEN upper(p_rule.use_estimate) = 'Y' THEN 'Y' ELSE 'N' END);
            END IF;
          ELSE
            ...
            ELSE
              UPDATE custm_rul_perd
                 SET period_list     = p_rule.period_list,
                     ...
                     use_estimate = CASE
                                      WHEN upper(p_rule.use_estimate) = 'Y' THEN
                                       'Y'
                                      ELSE
                                       'N'
                                    END
  ...
  BEGIN
    ...
    USING (SELECT p_mrkt_id AS mrkt_id,
                  (SELECT MAX(tc_bi24.trgt_sls_perd_id)
                     FROM (SELECT * FROM temp_config WHERE INSTR(c_sls_typ_grp_nm_bi24, sls_typ_grp_nm) > 0) tc_bi24,
                          ta_dict td
                    WHERE td.lbl_id = tc_bi24.offst_lbl_id
  ...
  EXCEPTION
    WHEN OTHERS THEN
      ...
      app_plsql_log.info(l_module_name ||
                         ...
                         ' error message: ' || SQLERRM || l_parameter_list);
      p_stus := 2;
  END;
    ...
    BEGIN
      ...
      */
      ...
      DELETE FROM dstrbtd_mrkt_sls
       WHERE (mrkt_id, sls_perd_id, offr_perd_id, sls_typ_id) IN
             (SELECT DISTINCT p_mrkt_id,
                              ...
                              p_sls_typ_id
                FROM temp_config 
               WHERE INSTR(c_sls_typ_grp_nm_fc_dbt || ', ' || c_sls_typ_grp_nm_fc_dms, sls_typ_grp_nm) > 0);
      l_deleted_rows := SQL%ROWCOUNT;
      app_plsql_log.set_context(l_user_id, g_package_name, l_run_id);
      app_plsql_log.info(l_module_name || ' ' || l_deleted_rows ||
      ...
      FOR i IN (SELECT DISTINCT offst_lbl_id,
                                ...
                                trgt_offr_perd_id
                  FROM temp_config 
                 WHERE INSTR(c_sls_typ_grp_nm_fc_dbt || ', ' || c_sls_typ_grp_nm_fc_dms, sls_typ_grp_nm) > 0) LOOP
      ...
      IF l_tbl_reproc_trnd.count > 0 THEN
        FOR i IN l_tbl_reproc_trnd.first .. l_tbl_reproc_trnd.last LOOP
          ...
          l_tbl_reproc(c_key).comsn_amt := nvl(l_tbl_reproc(c_key).comsn_amt,
                                               0) + l_tbl_reproc_trnd(i)
                                          .comsn_amt;
          l_tbl_reproc(c_key).tax_amt := nvl(l_tbl_reproc(c_key).tax_amt, 0) + l_tbl_reproc_trnd(i)
                                        .tax_amt;
        END LOOP;
        l_tbl_reproc_trnd.delete;
      END IF;
      ...
      IF l_tbl_reproc_trnd_fsc_cd.count > 0 THEN
        FOR i IN l_tbl_reproc_trnd_fsc_cd.first .. l_tbl_reproc_trnd_fsc_cd.last LOOP
          ...
          l_tbl_reproc(c_key).offr_sku_line_id := l_tbl_reproc_trnd_fsc_cd(i)
                                                  .offr_sku_line_id;
          l_tbl_reproc(c_key).offst_lbl_id := l_tbl_reproc_trnd_fsc_cd(i)
                                              .offst_lbl_id;
          l_tbl_reproc(c_key).sls_typ_lbl_id := l_tbl_reproc_trnd_fsc_cd(i)
                                                .sls_typ_lbl_id;
          l_tbl_reproc(c_key).units := nvl(l_tbl_reproc(c_key).units, 0) + l_tbl_reproc_trnd_fsc_cd(i)
                                      .units;
          l_tbl_reproc(c_key).sales := nvl(l_tbl_reproc(c_key).sales, 0) + l_tbl_reproc_trnd_fsc_cd(i)
                                      .sales;
          l_tbl_reproc(c_key).veh_id := l_tbl_reproc_trnd_fsc_cd(i).veh_id;
          l_tbl_reproc(c_key).comsn_amt := nvl(l_tbl_reproc(c_key).comsn_amt,
                                               0) + l_tbl_reproc_trnd_fsc_cd(i)
                                          .comsn_amt;
          l_tbl_reproc(c_key).tax_amt := nvl(l_tbl_reproc(c_key).tax_amt, 0) + l_tbl_reproc_trnd_fsc_cd(i)
                                        .tax_amt;
        END LOOP;
        l_tbl_reproc_trnd_fsc_cd.delete;
      END IF;
      ...
      IF l_tbl_reproc_est.count > 0 THEN
        FOR i IN l_tbl_reproc_est.first .. l_tbl_reproc_est.last LOOP
          ...
          l_tbl_reproc(c_key).comsn_amt := nvl(l_tbl_reproc(c_key).comsn_amt,
                                               0) + l_tbl_reproc_est(i)
                                          .comsn_amt;
          l_tbl_reproc(c_key).tax_amt := nvl(l_tbl_reproc(c_key).tax_amt, 0) + l_tbl_reproc_est(i)
                                        .tax_amt;
        END LOOP;
        l_tbl_reproc_est.delete;
      END IF;
      IF l_tbl_reproc.count > 0 THEN
        ...
        END LOOP;
        app_plsql_log.set_context(l_user_id, g_package_name, l_run_id);
        app_plsql_log.info(l_module_name || ' est (cycle): ' || l_cycl_cnt ||
                           ' est (insert): ' || l_ins_cnt ||
                           l_parameter_list);
        l_tbl_reproc.delete;
      END IF;
      -- end if;
      ...
      IF l_tbl_reproc_trendsetter.count > 0 THEN
        FOR i IN l_tbl_reproc_trendsetter.first .. l_tbl_reproc_trendsetter.last LOOP
          ...
          l_tbl_reproc(c_key).offr_sku_line_id := l_tbl_reproc_trendsetter(i)
                                                  .offr_sku_line_id;
          l_tbl_reproc(c_key).offst_lbl_id := l_tbl_reproc_trendsetter(i)
                                              .offst_lbl_id;
          l_tbl_reproc(c_key).sls_typ_lbl_id := l_tbl_reproc_trendsetter(i)
                                                .sls_typ_lbl_id;
          l_tbl_reproc(c_key).units := nvl(l_tbl_reproc(c_key).units, 0) + l_tbl_reproc_trendsetter(i)
                                      .units_forecasted;
          l_tbl_reproc(c_key).sales := nvl(l_tbl_reproc(c_key).sales, 0) + l_tbl_reproc_trendsetter(i)
                                      .sales_forecasted;
          l_tbl_reproc(c_key).veh_id := l_tbl_reproc_trendsetter(i).veh_id;
          l_tbl_reproc(c_key).comsn_amt := nvl(l_tbl_reproc(c_key).comsn_amt,
                                               0) + l_tbl_reproc_trendsetter(i)
                                          .comsn_amt;
          l_tbl_reproc(c_key).tax_amt := nvl(l_tbl_reproc(c_key).tax_amt, 0) + l_tbl_reproc_trendsetter(i)
                                        .tax_amt;
        END LOOP;
        l_tbl_reproc_trendsetter.delete;
      END IF;
      IF l_tbl_reproc.count > 0 THEN
        ...
        END LOOP;
        app_plsql_log.set_context(l_user_id, g_package_name, l_run_id);
        app_plsql_log.info(l_module_name || ' trendsetter (cycle): ' ||
                           l_cycl_cnt || ' trendsetter (insert): ' ||
                           l_ins_cnt || l_parameter_list);
        l_tbl_reproc.delete;
      END IF;
      --
      -- SAVE with l_spec_sls_typ_id the same records (which was processed in (trnd, trnd_fsc_cd, est, trendsetter)
      --
      -- set l_spec_sls_typ_id (versioned special sales type ID) for SAVE
      SELECT CASE
               WHEN p_sls_typ_id = supply_est_id THEN
                sc_est_day_0
               WHEN p_sls_typ_id = supply_est_id THEN
                sc_bst_day_0
               WHEN p_sls_typ_id = marketing_est_id THEN
                est_day_0
               WHEN p_sls_typ_id = marketing_bst_id THEN
                bst_day_0
       ...
       WHERE trnd_bilng_days.mrkt_id = p_mrkt_id
         ...
         AND trnd_bilng_days.prcsng_dt = p_bilng_day;
      IF l_spec_sls_typ_id >= least(supply_est_id,
                                    supply_est_id,
                                    marketing_est_id,
                                    marketing_bst_id) THEN
        ...
        app_plsql_log.set_context(l_user_id, g_package_name, l_run_id);
        app_plsql_log.info(l_module_name || ' ' || l_deleted_rows ||
                           ' records DELETED from dstrbtd_mrkt_sls (l_spec_sls_typ_id: ' ||
                           to_char(l_spec_sls_typ_id) || ') ' ||
                           l_parameter_list);
        ...
        BEGIN
          ...
          app_plsql_log.set_context(l_user_id, g_package_name, l_run_id);
          app_plsql_log.info(l_module_name || ' (l_spec_sls_typ_id: ' ||
                             to_char(l_spec_sls_typ_id) ||
                             ', based on p_sls_typ_id: ' ||
                             to_char(p_sls_typ_id) ||
                             ') spec_sls_typ_ID (insert): ' || l_ins_cnt ||
                             l_parameter_list);
        EXCEPTION
          WHEN OTHERS THEN
            app_plsql_log.error('ERROR at INSERT INTO dstrbtd_mrkt_sls (S) l_spec_sls_typ_id: ' ||
                                to_char(l_spec_sls_typ_id) || ', ' ||
...
END save_trend_alloctn;
    ...
    IF p_stus = 0 THEN
      ...
      BEGIN
        SELECT sct_autclc_ind, trnd_sls_perd_id
          INTO l_autclc_ind, l_sls_perd_id_mrkttrndslsprd
          FROM mrkt_trnd_sls_perd_sls_typ
         WHERE mrkt_id = p_mrkt_id
           AND trnd_sls_perd_id =
               (SELECT MAX(tc_bi24.trgt_sls_perd_id)
  ...
  BEGIN
    ...
    FOR rec IN (SELECT mrkt_trnd_sls_perd_sls_typ.mrkt_id,
                  ...
                  FROM mrkt_config_item, mrkt_trnd_sls_perd_sls_typ
                 WHERE mrkt_config_item.mrkt_config_item_desc_txt =
                       'PA_TREND_ALLOCATION ENABLED'
                   AND mrkt_config_item.mrkt_config_item_val_txt = 'Y'
                   AND mrkt_config_item.mrkt_id =
                       mrkt_trnd_sls_perd_sls_typ.mrkt_id
  ...
  BEGIN
    ...
    IF control_parameters_ok THEN
            ...
            FOR osl_rec IN (SELECT offr_sku_line_id,
                                   ...
                                   CASE
                                     WHEN duplicate_line_nr = 1 THEN
                                      'N'
                                     WHEN nvl(osl_line_nr, -1) = nvl(db_rec.bilng_line_nr, -1) THEN
                                      'Y'
                                     ELSE
                                      'N'
                                   END line_nr_match,
      ...
      app_plsql_log.set_context(l_user_id, g_package_name, l_run_id);
      app_plsql_log.info(l_module_name ||
                         ' Processing multi match fix(UNIT_QTY = dbt.unit_qty / osl count): ' ||
                         l_parameter_list);
      FOR mm_fix IN (SELECT dly_bilng_trnd.dly_bilng_id,
        ...
        UPDATE dly_bilng_trnd_osl_temp
           SET unit_qty =
               (mm_fix.unit_qty - MOD(mm_fix.unit_qty, mm_fix.c)) /
               mm_fix.c
        ...
        IF MOD(mm_fix.unit_qty, mm_fix.c) > 0 THEN
          UPDATE dly_bilng_trnd_osl_temp
           ...
           WHERE dly_bilng_id = mm_fix.dly_bilng_id
             AND dly_bilng_trnd_osl_temp.offr_sku_line_id =
                 mm_fix.max_sku_id;
        END IF;
        UPDATE dly_bilng_trnd_osl_temp
    ...
    FOR rec IN (WITH db AS
                   (SELECT mrkt_id,
                    ...
                    WHERE mrkt_id IN
                          (SELECT mrkt_id
                             FROM mrkt_config_item
                            WHERE mrkt_config_item_desc_txt =
                                  'PA_TREND_ALLOCATION ENABLED'
...
' || rec.wghtd_avg_cost_amt || '
        ...
        ELSE
          ...
          EXCEPTION
            WHEN OTHERS THEN
              app_plsql_log.info('Insert OFFR_PRLF_PRC_POINT: ' || SQLERRM);
...
');
        ...
        app_plsql_log.info('UNPLND_OFFR_RECRDS REC:' || ' | ' ||
                           ...
                           rec.prfl_nm || ' | ' || rec.sls_prc_amt ||
                           ' | ' || rec.nr_for_qty || ' | ' || rec.sku_id ||
                           ' | ' || rec.sls_perd_id || ' | ' ||
                           rec.wghtd_avg_cost_amt || ' | ' ||
                           rec.comsn_typ_id || ' | ' || rec.comsn_amt ||
                           ' | ' || rec.tax_typ_id || ' | ' || rec.tax_amt ||
                           ' | ' || rec.gta_method_id || ' | ' ||
                           rec.gta_pct || ' | ' || rec.sls_cls_cd || ' | ' ||
                           rec.bilng_id || ' | ' || rec.demand_unit_qty ||
                           ' | ' || rec.billed_unit_qty || ' | ' ||
  ...
  WHILE v_key1 <= v_offr_id_prfl_cd.last LOOP
    ...
    FOR rec IN (SELECT
                ...
                -----
                  FROM (SELECT db.offr_prfl_prcpt_id, db.prfl_cd, sku.sku_id
