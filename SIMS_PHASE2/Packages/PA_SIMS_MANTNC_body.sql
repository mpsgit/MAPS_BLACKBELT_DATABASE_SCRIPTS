create or replace PACKAGE BODY pa_sims_mantnc AS
  ...
  FUNCTION get_cash_vlu_mantnc(p_mrkt_id       IN NUMBER,
    ...
    CURSOR cc IS
      SELECT obj_cash_val_mantnc_line( SLS_PERD_ID,
                                       CASH_VAL,
                                       SCT_R_FACTOR,
                                       UPDT_IND,
                                       LAST_UPDT_TS,
                                       LAST_UPDT_USER_ID ) cline
       FROM (SELECT DISTINCT SLS_PERD_ID,
                             CASH_VAL,
                             SCT_R_FACTOR,
                             UPDT_IND,
                             LAST_UPDT_TS,
                             LAST_UPDT_USER_ID
               FROM (SELECT t.mrkt_id as MRKT_ID,
                            t.perd_id as SLS_PERD_ID,
                            tsp.sct_cash_value_on as CASH_VAL,
                            NULL as SCT_R_FACTOR,
                            (CASE
                              ...
                              ELSE
                               'PLANNING'
                            END) as UPDT_IND,
                            tsp.last_updt_ts as LAST_UPDT_TS,
                            u.user_frst_nm || ' ' || u.user_last_nm as LAST_UPDT_USER_ID
                      FROM (SELECT MAX(sls_perd_id) AS max_perd_id
                              FROM dly_bilng
                             WHERE mrkt_id = p_mrkt_id) m,
                           mrkt_perd t
                      LEFT JOIN mrkt_trnd_sls_perd_sls_typ tsp
                        ON tsp.mrkt_id = t.mrkt_id
                       AND tsp.trnd_sls_perd_id = t.perd_id
                      LEFT JOIN mps_user u
                        ON u.user_nm = tsp.last_updt_user_id
                       AND u.sys_id = 1
                     WHERE t.perd_typ = 'SC'
                       AND t.mrkt_id = p_mrkt_id
                       AND t.perd_id IN ((SELECT column_value FROM TABLE(p_sls_perd_list))))
               ORDER BY SLS_PERD_ID DESC );
    ...
    CURSOR cc IS
      SELECT obj_cash_val_mantnc_line(perd_id,
        ...
        FROM (SELECT t.perd_id,
                     tsp.sct_cash_value_on as sct_cash_val,
                     NULL as sct_r_factor,
                     CASE
                       WHEN m.max_perd_id > t.perd_id THEN
                        'FINISHED_TRENDING'
                ...
                FROM (SELECT MAX(sls_perd_id) AS max_perd_id
                       ...
                       WHERE mrkt_id = p_mrkt_id) m,
                     mrkt_perd t
                LEFT JOIN mrkt_trnd_sls_perd_sls_typ tsp -- actual values
                  ON tsp.mrkt_id = t.mrkt_id
                 AND tsp.trnd_sls_perd_id = t.perd_id
                LEFT JOIN mps_user u
                  ON u.user_nm = tsp.last_updt_user_id
  ...
  PROCEDURE set_cash_vlu_mantnc(p_mrkt_id     IN NUMBER,
    ...
    * 2 - database error in UPDATE MRKT_TRND_SLS_PERD_SLS_TYP or INSERT INTO CASH_VAL_RF_HIST statements
    * 3 - record to update in table MRKT_TRND_SLS_PERD_SLS_TYP is readonly (has records in DLY_BILNG)
    * 4 - period to update is not defined yet in table MRKT_PERD
    ******************************************************/
    max_perd_id           mrkt_trnd_sls_perd_sls_typ.trnd_sls_perd_id%TYPE;
    local_prcsng_dt       DATE := NULL;
    old_sct_cash_val      NUMBER;
    old_sct_r_factor      NUMBER;
    old_last_updt_user_id mrkt_trnd_sls_perd_sls_typ.last_updt_user_id%TYPE;
  ...
  BEGIN
    ...
    ELSE
      ...
      IF p_sls_perd_id > max_perd_id THEN
        ...
        IF mrkt_perd_exists > 0 THEN
          BEGIN
            SELECT DISTINCT sct_cash_value_on, NULL, last_updt_user_id
          ...
          EXCEPTION
            WHEN OTHERS THEN 
              old_sct_cash_val := NULL;
              old_sct_r_factor := NULL;
              old_last_updt_user_id := NULL;
          END;
          ...
          BEGIN
            ...
            IF wtd = 'U' THEN
              UPDATE mrkt_trnd_sls_perd_sls_typ
                 SET sct_cash_value_on      = p_cash_val,
                     --sct_r_factor        = r_factor_to_set,
                     sct_prcsng_dt       = local_prcsng_dt,
                     last_updt_user_id   = p_user_id
               WHERE mrkt_id = p_mrkt_id
                 AND trnd_sls_perd_id = p_sls_perd_id
                 AND sls_typ_id IN (SELECT DISTINCT trgt_sls_typ_id
                                     FROM ta_config
                                    WHERE mrkt_id = p_mrkt_id
                                      AND eff_sls_perd_id >= p_sls_perd_id);
            ELSE
              FOR i IN (SELECT DISTINCT trgt_sls_typ_id
                          FROM ta_config
                         WHERE mrkt_id = p_mrkt_id
                           AND eff_sls_perd_id >= p_sls_perd_id) LOOP
                INSERT INTO mrkt_trnd_sls_perd_sls_typ
                  (mrkt_id,
                   trnd_sls_perd_id,
                   sls_typ_id,
                   sct_cash_value_on,
  ...
  BEGIN
    p_stus := 0;
    -- p_new_sku_bias IS NULL or p_new_sku_bias = 100 - means do nothing
    IF nvl(p_new_sku_bias, 100) = 100 THEN
      NULL;
    ...
    ELSIF p_new_sku_bias BETWEEN 0 AND 1000 THEN
      ...
      END IF; --ccc3
    ELSE
      p_stus := 1; -- p_new_sku_bias is out of RANGE of (0 - 1000)
    END IF; -- 1 1000
  ...
  FUNCTION get_throw_forwrd_prct(p_mrkt_id   IN NUMBER,
    ...
    PIPELINED AS
    -- local variables
    l_perd_from NUMBER := p_perd_list(1);
    l_perd_to   NUMBER := p_perd_list(p_perd_list.last);
    -- for LOG
    l_run_id  NUMBER := app_plsql_output.generate_new_run_id;
    l_user_id VARCHAR(35) := USER();
    --
    l_module_name    VARCHAR2(30) := 'GET_THROW_FORWRD_PRCT';
    l_parameter_list VARCHAR2(2048) := '(p_mrkt_id: ' || to_char(p_mrkt_id) || ', ' ||
                                       'p_perd_list: ' || to_char(l_perd_from) || ' - ' || to_char(l_perd_to) ||
                                       'p_user_id: ' || l_user_id || ')';
    --
  BEGIN
    app_plsql_log.register(g_package_name || '.' || l_module_name);
    ...
    app_plsql_log.set_context(l_user_id, g_package_name, l_run_id);
    app_plsql_log.info(l_module_name || l_parameter_list || ' start');
    FOR rec IN (SELECT mrkt_perd.mrkt_id,
                       mrkt_perd.perd_id,
                       mrkt_perd_throw_forwrd_prct.throw_forwrd_pct,
                       mrkt_perd_throw_forwrd_prct.last_updt_user_id,
                       mrkt_perd_throw_forwrd_prct.last_updt_ts,
                       mrkt_perd_throw_forwrd_prct.creat_user_id,
                       mrkt_perd_throw_forwrd_prct.creat_ts
                  FROM mrkt_perd,
                       mrkt_perd_throw_forwrd_prct
                 WHERE mrkt_perd.mrkt_id = p_mrkt_id
                   AND mrkt_perd.perd_id IN (SELECT column_value FROM TABLE(p_perd_list))
                   AND mrkt_perd.perd_typ = 'SC'
                   AND mrkt_perd.mrkt_id = mrkt_perd_throw_forwrd_prct.mrkt_id(+) 
                   AND mrkt_perd.perd_id = mrkt_perd_throw_forwrd_prct.perd_id(+)
                 ORDER BY mrkt_perd.mrkt_id,
                          mrkt_perd.perd_id) LOOP
      PIPE ROW(obj_mrktprdthrwfrwrdprct_line(rec.perd_id,
                                             rec.throw_forwrd_pct,
  ...
  PROCEDURE set_throw_forwrd_prct(p_mrkt_id               IN NUMBER,
    ...
    ******************************************************/
    ...
    l_module_name    VARCHAR2(30) := 'SET_THROW_FORWRD_PRCT';
    l_parameter_list VARCHAR2(2048) := '(p_mrkt_id: ' || to_char(p_mrkt_id) || ', ' ||
                                       'p_perd_list: ' || to_char(l_perd_from) || ' - ' || to_char(l_perd_to) ||
                                       'p_user_id: ' || l_user_id || ')';
    --
  BEGIN
    ...
    --
    BEGIN
      MERGE INTO mrkt_perd_throw_forwrd_prct t
      USING (SELECT p_mrkt_id as mrkt_id,
                    perd_id,
                    throw_forwrd_pct
               FROM TABLE(p_throw_forwrd_prct_lst)) x
      ON (t.mrkt_id = x.mrkt_id AND t.perd_id = x.perd_id)
      WHEN MATCHED THEN
        UPDATE SET t.throw_forwrd_pct = x.throw_forwrd_pct,
                   t.last_updt_user_id = l_user_id
      WHEN NOT MATCHED THEN
        INSERT
          (mrkt_id, perd_id, throw_forwrd_pct, creat_user_id, last_updt_user_id)
        VALUES
          (x.mrkt_id, x.perd_id, x.throw_forwrd_pct, l_user_id, l_user_id);
    EXCEPTION
      WHEN OTHERS THEN
        app_plsql_log.set_context(l_user_id, g_package_name, l_run_id);
        app_plsql_log.info(l_module_name ||
                           ' merge into MRKT_PERD_THROW_FORWRD_PRCT for mrkt_id=' || to_char(p_mrkt_id) ||
                           ' and p_perd_list: ' || to_char(l_perd_from) || ' - ' || to_char(l_perd_to) ||
    ...
    --
    app_plsql_log.set_context(l_user_id, g_package_name, l_run_id);
    app_plsql_log.info(l_module_name || ' end, status_code: ' || to_char(p_stus) || l_parameter_list);
  END set_throw_forwrd_prct;
