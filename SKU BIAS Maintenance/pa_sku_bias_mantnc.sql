  ...
  CURSOR cc IS
    WITH MPSB AS
      ...
      FROM MRKT_PERD_SKU_BIAS PSB
        LEFT JOIN MPS_USER U ON PSB.LAST_UPDT_USER_ID=U.USER_NM
      WHERE PSB.MRKT_ID=p_mrkt_id AND PSB.SLS_PERD_ID=p_sls_perd_id
      ),
      ACT_FSC AS
      (SELECT MRKT_FSC.SKU_ID,
        MAX(MRKT_FSC.STRT_PERD_ID) MAX_PERD_ID
      FROM MRKT_FSC
      WHERE MRKT_FSC.MRKT_ID=p_mrkt_id AND MRKT_FSC.STRT_PERD_ID<= p_sls_perd_id
      GROUP BY MRKT_FSC.SKU_ID
      ),
      ALL_FSC AS
      (SELECT MRKT_FSC.STRT_PERD_ID, MRKT_FSC.FSC_CD, MRKT_FSC.SKU_ID,
        MRKT_FSC.PROD_DESC_TXT
      FROM MRKT_FSC
        WHERE MRKT_FSC.STRT_PERD_ID<=p_sls_perd_id AND MRKT_FSC.MRKT_ID=p_mrkt_id
      )
    SELECT 
    ...
    FROM ACT_FSC, ALL_FSC, MPSB,
      (SELECT SMS.MRKT_ID,
        SMS.SKU_ID,SMS.LCL_SKU_NM
      FROM
        (SELECT mrkt_sku.MRKT_ID AS MRKT_ID,
          mrkt_sku.SKU_ID, mrkt_sku.LCL_SKU_NM
        FROM mrkt_sku
  ...
  ******************************************************/
  ...
  BEGIN
    ...
    IF c_new_sku_bias between 0 AND 1000 THEN
      ...
      SELECT PH, CASE WHEN PA_MAPS_PUBLIC.get_sls_cls_cd(p_sls_perd_id, p_mrkt_id, s.avlbl_perd_id,
        ...
        FROM (select 1 PH from dual) DD 
          left join MRKT_SKU S
            ON S.MRKT_ID=p_mrkt_id AND S.SKU_ID=p_sku_id;
          ...
          IF counter1+counter2=2 THEN
            ...
            MERGE INTO MRKT_PERD_SKU_BIAS trgt
              ...
              WHEN NOT MATCHED THEN
                INSERT (MRKT_ID,SLS_PERD_ID,SKU_ID,BIAS_PCT,CREAT_USER_ID,LAST_UPDT_USER_ID)
                  VALUES (p_mrkt_id,p_sls_perd_id,p_sku_id,c_new_sku_bias,p_user_id,p_user_id);
