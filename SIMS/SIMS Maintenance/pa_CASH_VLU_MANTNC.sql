CREATE OR REPLACE PACKAGE CASH_VLU_MANTNC IS
  ...
  ******************************************************/
  ...
  FUNCTION GET_CASH_VAL_HIST(p_mrkt_id IN NUMBER,
                        p_sls_perd_list IN NUMBER_ARRAY
                        ) RETURN OBJ_CASH_VAL_MANTNC_TABLE PIPELINED;                                
...
END;
  ...
  function get_cash_val_hist(p_mrkt_id in number,
                        p_sls_perd_list in number_array
                        ) return obj_cash_val_mantnc_table pipelined as
    ...
    l_MAX_PERD_ID NUMBER;
    signature VARCHAR(100) := 'GET_CASH_VLU_MANTNC_HIST(p_mrkt_id: ' || p_mrkt_id || ', ' ;
  BEGIN
    ...
    SELECT  MAX(SLS_PERD_ID) INTO l_MAX_PERD_ID
    FROM DLY_BILNG WHERE MRKT_ID=p_mrkt_id;
    for i in (SELECT 
                T.PERD_ID,
                SP.SCT_CASH_VAL cash_val,
                SP.SCT_R_FACTOR r_factor,
                CASE
                  WHEN l_MAX_PERD_ID>t.PERD_ID THEN 'FINISHED_TRENDING'
                  WHEN l_MAX_PERD_ID=t.PERD_ID THEN 'CURRENT_TRENDING'
                  ELSE 'PLANNING'
                END trend_status,
                SP.LAST_UPDT_TS,
