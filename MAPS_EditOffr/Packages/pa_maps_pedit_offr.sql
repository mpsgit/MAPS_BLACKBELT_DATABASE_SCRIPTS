CREATE OR REPLACE PACKAGE PA_MAPS_PEDIT_OFFR
AS
   /************************************************************************************************************
   ...
   ************************************************************************************************************/
   ...
   g_package_name             CONSTANT VARCHAR2 (30) := 'PA_MAPS_PEDIT_OFFR';
   ...
   FUNCTION get_scenario_list (p_mrkt_id        IN number_array,
                               ...
                               p_veh_id         IN number_array)
      RETURN obj_scenario_table
      PIPELINED;

   FUNCTION get_edit_offr_table (p_filters      IN obj_pedit_offr_filter_table,
                                 p_pagination   IN CHAR DEFAULT 'N')
      RETURN OBJ_PEDIT_OFFR_TABLE
      PIPELINED;
   ...
   FUNCTION get_history (p_get_offr IN obj_get_offr_table)
      RETURN OBJ_PEDIT_OFFR_HIST_TABLE
      PIPELINED;

   PROCEDURE save_edit_offr_table (
      p_data_line    IN     OBJ_PEDIT_OFFR_TABLE,
   ...
   PROCEDURE merge_history (p_get_offr IN OBJ_PEDIT_OFFR_TABLE);

   PROCEDURE lock_offr (p_lock_offr   IN     obj_edit_offr_lock_table,
                        r_lock_offr      OUT obj_edit_offr_lock_res_table);

   PROCEDURE unlock_offr (
      p_unlock_offr   IN     obj_edit_offr_lock_table,
      r_unlock_offr      OUT obj_edit_offr_lock_res_table);

   FUNCTION get_sls_typ_and_grp (
      p_mrkt_perd   IN obj_edit_offr_mrkt_perd_table)
   ...
   PROCEDURE add_offer (p_mrkt_id                 IN     NUMBER,
                        ...
                        p_status                     OUT NUMBER,
                        p_edit_offr_table            OUT OBJ_PEDIT_OFFR_TABLE,
                        p_pagination              IN     CHAR DEFAULT 'N');

   PROCEDURE add_concepts_to_offr (
      p_offr_id           IN     NUMBER,
      p_prfl_cd_list      IN     number_array,
      p_user_nm           IN     VARCHAR2,
      p_clstr_id          IN     NUMBER,
   ...
   PROCEDURE add_prcpoints_to_offr (
      p_offr_id                   IN     NUMBER,
   ...
   PROCEDURE copy_offer (p_copy_offr_table   IN     obj_pcopy_offr_table,
                         p_user_nm           IN     VARCHAR2,
                         p_status               OUT NUMBER,
                         p_edit_offr_table      OUT OBJ_PEDIT_OFFR_TABLE,
                         p_pagination        IN     CHAR DEFAULT 'N');

   PROCEDURE delete_offers (p_osl_records       IN     OBJ_PEDIT_OFFR_TABLE,
                            p_edit_offr_table      OUT OBJ_PEDIT_OFFR_TABLE);

   PROCEDURE delete_prcpoints (
      p_osl_records       IN     OBJ_PEDIT_OFFR_TABLE,
      p_edit_offr_table      OUT OBJ_PEDIT_OFFR_TABLE,
      p_pagination        IN     CHAR DEFAULT 'N');

   PROCEDURE copy_prcpts_to_offr (
      p_offr_prfl_prcpt_ids   IN     number_array,
...
END PA_MAPS_PEDIT_OFFR;
/
CREATE OR REPLACE PACKAGE BODY PA_MAPS_PEDIT_OFFR
AS
   ...
   BEGIN
      ...
      FOR rec
         IN (SELECT scnrio_id, scnrio_desc_txt
               FROM what_if_scnrio
              WHERE     mrkt_id IN (SELECT * FROM TABLE (p_mrkt_id))
                    AND (   strt_perd_id IN (SELECT *
                                               FROM TABLE (p_offr_perd_id))
                         OR end_perd_id IN (SELECT *
                                              FROM TABLE (p_offr_perd_id)))
   ...
   BEGIN
      ...
      INSERT INTO what_if_scnrio (mrkt_id,
                                  veh_id,
                                  scnrio_id,
                                  scnrio_desc_txt,
                                  strt_perd_id,
   ...
   BEGIN
      ...
      INSERT INTO what_if_tran (mrkt_id,
                                veh_id,
                                scnrio_id,
                                tran_id,
                                tran_typ,
   ...
   PROCEDURE copy_offr_add_to_scnrio (
      p_offr_id         IN     offr.offr_id%TYPE,
      ...
      p_new_offr_id        OUT offr.offr_id%TYPE)
   IS
   ...
   BEGIN
      ...
      p_new_offr_id :=
         pa_maps_copy.copy_offer (par_offerid          => p_offr_id,
   ...
   EXCEPTION
      ...
      THEN
         app_plsql_log.info (
            l_module_name || ' ' || l_log || ' ' || SQLERRM (SQLCODE));
         RAISE;
   END copy_offr_add_to_scnrio;

   PROCEDURE create_wif_offrs_wif (
      p_user_nm          IN            VARCHAR2,
      ...
      p_veh_id           IN            offr.veh_id%TYPE,
      p_scnrio_id        IN            what_if_scnrio.scnrio_id%TYPE,
      p_get_offr_table   IN OUT NOCOPY obj_get_offr_table)
   IS
   ...
   BEGIN
      ...
      FOR offr_rec
         IN (SELECT o.offr_id, o.offr_desc_txt, o.offr_typ
               FROM offr o
              WHERE     NOT EXISTS
                               (SELECT 1
                                  FROM offr o2, what_if_tran t
                                 WHERE     t.offr_id = o2.offr_id
                                       ...
                                       AND o2.mrkt_veh_perd_sctn_id =
                                              o.mrkt_veh_perd_sctn_id
                                       AND o2.sctn_page_ofs_nr =
                                              o.sctn_page_ofs_nr
      ...
      LOOP
         app_plsql_log.info (
               'create_wif_offrs_wif loop:'
            || '-> '
            || offr_rec.offr_id
            || '-> '
            || offr_rec.offr_desc_txt);

         copy_offr_add_to_scnrio (offr_rec.offr_id,
                                  offr_rec.offr_desc_txt,
                                  p_user_nm,
                                  p_mrkt_id,
                                  p_offr_perd_id,
         ...
         p_get_offr_table (p_get_offr_table.LAST) :=
            obj_get_offr_line (l_new_offr_id, 1);
      END LOOP;
   ...
   PROCEDURE create_wif_offrs_cmp (
      p_offr_id          IN            NUMBER,
      p_user_nm          IN            VARCHAR2,
      p_mrkt_id          IN            offr.mrkt_id%TYPE,
      p_offr_perd_id     IN            offr.offr_perd_id%TYPE,
   ...
   BEGIN
      ...
      IF l_offr_typ = 'CMP' AND l_scnrio_id_arr.COUNT > 0
      THEN
         l_log := 'Creating WIF offers for CMP offer';

         FOR scnr_ind IN l_scnrio_id_arr.FIRST .. l_scnrio_id_arr.LAST
         LOOP
            create_wif_offrs_cmp (p_offr_id,
                                  p_user_nm,
                                  l_mrkt_id,
                                  l_offr_perd_id,
                                  l_veh_id,
                                  l_scnrio_id_arr (scnr_ind),
                                  p_get_offr_table);
         END LOOP;
      ELSIF l_offr_typ = 'WIF' AND l_scnrio_id IS NOT NULL
      THEN
         l_log := 'Creating WIF offers for WIF offer';
         create_wif_offrs_wif (p_user_nm,
                               l_mvps_id,
                               l_pg_nr,
                               l_mrkt_id,
                               l_offr_perd_id,
   ...
   PROCEDURE get_offers_in_scnrio (
      p_offr_id          IN            NUMBER,
      p_scnrio_id        IN            NUMBER,
      p_mrkt_id          IN            NUMBER,
      p_offr_perd_id     IN            NUMBER,
   ...
   BEGIN
      ...
      LOOP
         p_get_offr_table.EXTEND;
         p_get_offr_table (p_get_offr_table.LAST) :=
            obj_get_offr_line (offr_rec.offr_id, g_sls_typ_id);
      END LOOP;
   ...
   FUNCTION lock_offr_chk (p_offr_id IN NUMBER, p_user_nm IN VARCHAR2)
      RETURN NUMBER
   AS
      ln_status   NUMBER;
   BEGIN
      SELECT COUNT (*)
        INTO ln_status
        FROM offr_lock
       WHERE offr_lock.offr_id = p_offr_id AND offr_lock.user_nm = p_user_nm;
   ...
   BEGIN
      BEGIN
         SELECT COUNT (*)
           INTO ln_lock
           FROM offr_lock
          WHERE offr_id = p_offr_id AND clstr_id = p_clstr_id;
      EXCEPTION
         WHEN NO_DATA_FOUND
         THEN
            ln_lock := 0;
   ...
   BEGIN
      ...
      LOOP
         BEGIN
            r_lock_offr.EXTEND ();
            r_lock_offr (r_lock_offr.LAST) :=
               obj_edit_offr_lock_res_line (NULL, rec.offr_id, NULL);
            lock_offr (rec.offr_id,
                       rec.user_nm,
                       rec.clstr_id,
                       r_lock_offr (r_lock_offr.LAST).lock_user_nm,
                       r_lock_offr (r_lock_offr.LAST).status);
         END;
      END LOOP;
   EXCEPTION
      WHEN OTHERS
      THEN
         app_plsql_log.info (
            'Error while bulk locking, error_msg:' || SQLERRM (SQLCODE));
   END lock_offr;
      ...
      THEN
         ...
         EXCEPTION
            ...
            THEN
               ...
               p_lock_user_nm := NULL;
               app_plsql_log.info (
                     'Error while unlocking offer_id: '
                  || TO_CHAR (p_offr_id)
                  || ' user name: '
                  || p_user_nm
                  || ' error_msg:'
   ...
   BEGIN
      ...
      LOOP
         BEGIN
            ...
            r_unlock_offr (r_unlock_offr.LAST) :=
               obj_edit_offr_lock_res_line (NULL, rec.offr_id, NULL);
            unlock_offr (rec.offr_id,
                         rec.user_nm,
                         r_unlock_offr (r_unlock_offr.LAST).lock_user_nm,
                         r_unlock_offr (r_unlock_offr.LAST).status);
         END;
      END LOOP;
   EXCEPTION
      WHEN OTHERS
      THEN
         app_plsql_log.info (
            'Error while bulk unlocking, error_msg:' || SQLERRM (SQLCODE));
   END unlock_offr;
   ...
   BEGIN
      MERGE INTO EDIT_OFFR_HIST hist
           USING (SELECT *
                    FROM TABLE (PA_MAPS_PEDIT_OFFR.get_offr (p_get_offr))) cur
              ON (    NVL (hist.intrnl_offr_id, -1) =
                         NVL (cur.intrnl_offr_id, -1)
                  AND NVL (hist.offr_sku_line_id, -1) =
                         NVL (cur.offr_sku_line_id, -1)
                  AND NVL (hist.status, -1) = NVL (cur.status, -1)
                  AND NVL (hist.mrkt_id, -1) = NVL (cur.mrkt_id, -1)
                  AND NVL (hist.offr_perd_id, -1) =
                         NVL (cur.offr_perd_id, -1)
                  AND NVL (hist.offr_lock, -1) = NVL (cur.offr_lock, -1)
                  AND NVL (hist.offr_lock_user, -1) =
                         NVL (cur.offr_lock_user, -1)
                  AND NVL (hist.veh_id, -1) = NVL (cur.veh_id, -1)
                  AND NVL (hist.brchr_plcmnt_id, -1) =
                         NVL (cur.brchr_plcmnt_id, -1)
                  AND NVL (hist.brchr_sctn_nm, -1) =
                         NVL (cur.brchr_sctn_nm, -1)
                  AND NVL (hist.enrgy_chrt_postn_id, -1) =
                         NVL (cur.enrgy_chrt_postn_id, -1)
                  AND NVL (hist.web_postn_id, -1) =
                         NVL (cur.web_postn_id, -1)
                  ...
                  AND NVL (hist.pp_sls_cls_cd, -1) =
                         NVL (cur.pp_sls_cls_cd, -1)
                  AND NVL (hist.offr_desc_txt, -1) =
                         NVL (cur.offr_desc_txt, -1)
                  AND NVL (hist.offr_notes_txt, -1) =
                         NVL (cur.offr_notes_txt, -1)
                  AND NVL (hist.offr_lyot_cmnts_txt, -1) =
                         NVL (cur.offr_lyot_cmnts_txt, -1)
                  AND NVL (hist.featrd_side_cd, -1) =
                         NVL (cur.featrd_side_cd, -1)
                  AND NVL (hist.concept_featrd_side_cd, -1) =
                         NVL (cur.concept_featrd_side_cd, -1)
                  AND NVL (hist.micr_ncpsltn_ind, -1) =
                         NVL (cur.micr_ncpsltn_ind, -1)
                  AND NVL (hist.scntd_pg_typ_id, -1) =
                         NVL (cur.scntd_pg_typ_id, -1)
                  AND NVL (hist.cnsmr_invstmt_bdgt_id, -1) =
                         NVL (cur.cnsmr_invstmt_bdgt_id, -1)
                  AND NVL (hist.pymt_typ, -1) = NVL (cur.pymt_typ, -1)
                  AND NVL (hist.promtn_id, -1) = NVL (cur.promtn_id, -1)
                  AND NVL (hist.promtn_clm_id, -1) =
                         NVL (cur.promtn_clm_id, -1)
                  ...
                  AND NVL (hist.offr_sku_set_id, -1) =
                         NVL (cur.offr_sku_set_id, -1)
                  ...
                  AND NVL (hist.lv_total_cost, -1) =
                         NVL (cur.lv_total_cost, -1)
                  AND NVL (hist.lv_gross_sales, -1) =
                         NVL (cur.lv_gross_sales, -1)
                  AND NVL (hist.lv_dp_cash, -1) = NVL (cur.lv_dp_cash, -1)
                  AND NVL (hist.lv_dp_percent, -1) =
                         NVL (cur.lv_dp_percent, -1)
                  ...
                  AND NVL (hist.created_ts,
                           TO_DATE ('1989.02.15', 'yyyy.mm.dd')) =
                         NVL (cur.created_ts,
                              TO_DATE ('1989.02.15', 'yyyy.mm.dd'))
                  AND NVL (hist.created_user_id, -1) =
                         NVL (cur.created_user_id, -1)
                  AND NVL (hist.last_updt_ts,
                           TO_DATE ('1989.02.15', 'yyyy.mm.dd')) =
                         NVL (cur.last_updt_ts,
                              TO_DATE ('1989.02.15', 'yyyy.mm.dd'))
                  AND NVL (hist.last_updt_user_id, -1) =
                         NVL (cur.last_updt_user_id, -1)
                  AND NVL (hist.mrkt_veh_perd_sctn_id, -1) =
                         NVL (cur.mrkt_veh_perd_sctn_id, -1)
                  AND NVL (hist.prfl_nm, -1) = NVL (cur.prfl_nm, -1)
                  AND NVL (hist.sku_nm, -1) = NVL (cur.sku_nm, -1)
                  AND NVL (hist.comsn_typ_desc_txt, -1) =
                         NVL (cur.comsn_typ_desc_txt, -1)
                  AND NVL (hist.tax_typ_desc_txt, -1) =
                         NVL (cur.tax_typ_desc_txt, -1)
                  AND NVL (hist.offr_sku_set_nm, -1) =
                         NVL (cur.offr_sku_set_nm, -1)
                  ...
                  AND NVL (hist.offr_prfl_prcpt_id, -1) =
                         NVL (cur.offr_prfl_prcpt_id, -1)
                  AND NVL (hist.offr_typ, -1) = NVL (cur.offr_typ, -1)
                  AND NVL (hist.forcasted_units, -1) =
                         NVL (cur.forcasted_units, -1)
                  AND NVL (hist.forcasted_date,
                           TO_DATE ('1989.02.15', 'yyyy.mm.dd')) =
                         NVL (cur.forcasted_date,
                              TO_DATE ('1989.02.15', 'yyyy.mm.dd'))
                  AND NVL (hist.offr_cls_id, -1) = NVL (cur.offr_cls_id, -1)
                  AND NVL (hist.spcl_ordr_ind, -1) =
                         NVL (cur.spcl_ordr_ind, -1)
                  AND NVL (hist.offr_ofs_nr, -1) = NVL (cur.offr_ofs_nr, -1)
                  AND NVL (hist.pp_ofs_nr, -1) = NVL (cur.pp_ofs_nr, -1)
                  AND NVL (hist.impct_catgry_id, -1) =
                         NVL (cur.impct_catgry_id, -1)
                  ...
                  AND NVL (hist.micr_ncpsltn_desc_txt, -1) =
                         NVL (cur.micr_ncpsltn_desc_txt, -1))
      WHEN NOT MATCHED
      THEN
         INSERT     (hist_ts,
                     intrnl_offr_id,
      ...
      app_plsql_log.info (
         'Edit offr history merge rows inserted: ' || TO_CHAR (l_rowcount));
   ...
   EXCEPTION
      ...
      THEN
         ...
         ROLLBACK;
         app_plsql_log.info (
            'Error in edit offr history merge ' || SQLERRM (SQLCODE));
   END set_history;
   ...
   BEGIN
      FOR rec IN (SELECT *
                   ...
                   WHERE intrnl_offr_id = p_offr_id AND sls_typ = p_sls_typ)
      LOOP
         app_plsql_log.info (
            SUBSTR (
                  p_module_name
   ...
   BEGIN
      ...
      FOR rec
         IN (SELECT osl.pg_ofs_nr, l.*
               FROM TABLE (p_data_line) l, offr_sku_line osl
      ...
      LOOP
         SELECT COUNT (*)
          ...
          WHERE osl.offr_id = p_offr_id AND osl.prfl_cd = rec.prfl_cd;
         ...
         IF l_cnt = 0
         THEN
            INSERT INTO offr_prfl_sls_cls_plcmt (offr_id,
                                                 sls_cls_cd,
                                                 prfl_cd,
                                                 pg_ofs_nr,
                                                 featrd_side_cd,
         ...
         IF l_cnt = 0
         THEN
            INSERT INTO offr_sls_cls_sku (offr_id,
                                          sls_cls_cd,
                                          prfl_cd,
                                          pg_ofs_nr,
                                          featrd_side_cd,
                 ...
                 VALUES (rec.intrnl_offr_id,
                         ...
                         'N',
                         'N',
                         DECODE (rec.scntd_pg_typ_id, NULL, 'N', 'Y'),
                         rec.scntd_pg_typ_id,
                         rec.reg_prc_amt,
                         NULL,
                         NULL,
         ...
         IF l_cnt = 0
         THEN
            INSERT INTO pg_wght (pg_wght_pct,
                                 mrkt_id,
                                 pg_wght_desc_txt,
                                 creat_user_id,
                                 last_updt_user_id)
         ...
         THEN
            INSERT INTO pg_wght (pg_wght_pct,
                                 ...
                                 creat_user_id,
                                 last_updt_user_id)
                 VALUES (rec.pp_pg_wght,
                         rec.mrkt_id,
                         TO_CHAR (rec.pp_pg_wght) || '%',
                         rec.offr_lock_user,
                         rec.offr_lock_user);
         END IF;
      END LOOP;
      ...
      FOR rec
         IN (SELECT l.pp_ofs_nr,
              ...
              WHERE l.intrnl_offr_id = p_offr_id AND l.sls_typ = p_sls_typ)
      LOOP
         BEGIN
            ...
            l_rowcount := SQL%ROWCOUNT;
            app_plsql_log.info (
               l_log || ' OSCS rows updated: ' || l_rowcount);
         EXCEPTION
      ...
      FOR r_data_line IN c_p_data_line
      LOOP
         l_no_sku_line := FALSE;
             ...
             WHERE     oscs.offr_id = r_data_line.intrnl_offr_id
                   ...
                   AND oscs.pg_ofs_nr =
                          (SELECT osl.pg_ofs_nr
                             FROM offr_sku_line osl
                            WHERE osl.offr_sku_line_id =
                                     r_data_line.offr_sku_line_id)
                   AND oscs.featrd_side_cd =
                          r_data_line.concept_featrd_side_cd
         ...
         IF l_no_sku_line = FALSE
         THEN
            SELECT oppp.sls_prc_amt, oppp.nr_for_qty, osl.set_cmpnt_qty
              INTO l_old_sls_prc_amt, l_old_nr_for_qty, l_old_cmpnt_qty
            ...
            IF    l_oss_id_to_zero = r_data_line.offr_sku_set_id
               OR (    l_cnt > 0
                   AND (   l_old_sls_prc_amt <> r_data_line.sls_prc_amt
                        ...
                        OR NVL (l_old_micr_ncpsltn_ind, -1) <>
                              NVL (r_data_line.micr_ncpsltn_ind, -1))
            ...
            THEN
               ...
               UPDATE dstrbtd_mrkt_sls dms
                  SET dms.unit_qty = 0,
                      dms.tax_amt =
                         pa_maps_gta.pri_get_tax_amount (
                            dms.mrkt_id,
                      ...
                      AND CASE
                             ...
                             WHEN     r_data_line.offr_sku_set_id IS NOT NULL
                                  AND dms.offr_sku_line_id IN (SELECT offr_sku_line_id
                                                                 FROM offr_sku_line
                                                                WHERE offr_sku_set_id =
                                                                         r_data_line.offr_sku_set_id)
                             THEN
                                1
                             ELSE
                                0
      ...
      THEN
         ...
         THEN
            UPDATE SET
               dms.unit_qty = dl.unit_qty,
               dms.tax_amt =
                  pa_maps_gta.pri_get_tax_amount (dms.mrkt_id,
                                                  dl.tax_type_id,
                                                  dms.offr_perd_id),
               dms.comsn_amt =
                  pa_maps_gta.get_commission_percentage (dms.mrkt_id,
                                                         dms.offr_perd_id,
                                                         dl.comsn_typ),
               dms.last_updt_user_id = dl.last_updt_user_id;

         -- Calculate the offset units
         pa_maps_gta.set_offset_units (poffer_id        => p_offr_id,
                                       psales_type_id   => p_sls_typ);
      END IF;
      ...
      WHEN MATCHED
      THEN
         UPDATE SET s.set_prc_amt = dl.sum_prc_amt,
      ...
      MERGE INTO offr o
           USING (SELECT DISTINCT
                         ...
                         (SELECT MAX (NVL (oscs.micr_ncpsltn_ind, 'N'))
                            FROM offr_sls_cls_sku oscs, offr_sku_line osl
      ...
      WHEN MATCHED
      THEN
         UPDATE SET o.enrgy_chrt_postn_id = dl.enrgy_chrt_postn_id,
      ...
      MERGE INTO offr_prfl_prc_point p
           USING (SELECT DISTINCT
                         ...
                         l.sls_prc_amt,
                         l.tax_type_id,
                         pa_maps_gta.pri_get_tax_amount (l.mrkt_id,
                                                         l.tax_type_id,
                                                         l.offr_perd_id)
                            AS tax_amt,
                         pa_maps_gta.get_commission_percentage (
                         ...
                         pa_maps_gta.get_gta_without_price_point (
                            pa_maps_gta.pri_get_gta_method_id (
                               oppp.mrkt_id,
                               oppp.offr_perd_id),
                            l.sls_prc_amt,
                            oppp.chrty_amt,
      ...
      WHEN MATCHED
      THEN
         UPDATE SET p.featrd_side_cd = dl.concept_featrd_side_cd,
      ...
      LOOP
         BEGIN
            SELECT cnt
              INTO l_cnt
              FROM (  SELECT osl.offr_prfl_prcpt_id, COUNT (*) cnt
                        FROM offr_sku_line osl
         ...
         IF l_cnt > 0
         THEN
            FOR r_osl IN (SELECT offr_sku_line_id
                            FROM offr_sku_line
            ...
            LOOP
               ...
               DELETE FROM est_lnkg
                     WHERE r_osl.offr_sku_line_id IN (offr_sku_line_id,
                                                      offr_gift_sku_line_id);
            END LOOP;
   ...
   AS
      ...
      l_get_offr_table   OBJ_PEDIT_OFFR_TABLE;
   ...
   BEGIN
      ...
      FOR mrkt_prd_rec
         IN (SELECT DISTINCT mrkt_id, offr_perd_id FROM TABLE (p_data_line))
      LOOP
         l_offr_table.delete;

         FOR rec
            IN (  SELECT intrnl_offr_id, sls_typ
                    FROM TABLE (p_data_line)
                   WHERE     mrkt_id = mrkt_prd_rec.mrkt_id
                         AND offr_perd_id = mrkt_prd_rec.offr_perd_id
                GROUP BY intrnl_offr_id, sls_typ)
         LOOP
            l_offr_table.EXTEND;
            l_offr_table (l_offr_table.LAST) :=
               obj_get_offr_line (rec.intrnl_offr_id, NVL (rec.sls_typ, 1));
         END LOOP;

         SELECT obj_pedit_offr_line (status,
                                    mrkt_id,
                                    offr_perd_id,
                                    offr_lock,
                                    offr_lock_user,
           ...
           FROM TABLE (
                   PA_MAPS_PEDIT_OFFR.get_offr (l_offr_table, p_pagination));

         merge_history (l_get_offr_table);

         -- check if the received osl -ids are the same as
         FOR offr_sls
            IN (  SELECT offr_id, sls_typ, MAX (diff) AS diff
                    FROM (SELECT offr_id, sls_typ, 1 AS diff -- different lines
         ...
         LOOP
            ...
            THEN
               ...
               IF l_status = 1
               THEN       -- Check if user has right to modify without locking
                  l_result := 1;
                  ...
                  BEGIN
                     ...
                     -- Calculate the offset units
                     pa_maps_gta.set_offset_units (
                        poffer_id        => offr_sls.offr_id,
                        psales_type_id   => offr_sls.sls_typ);

                     app_plsql_log.info (
                           'Setting units finished (without lock), merge rowcount: '
                        || l_rowcount);
                  EXCEPTION
                     WHEN OTHERS
                     THEN
                        ...
                        app_plsql_log.info (
                              l_module_name
                           || ', '
                           || 'Setting units finished (without lock), offr_id: '
                           || offr_sls.offr_id
                           || ', sls_typ: '
                           || offr_sls.sls_typ
                           || ', Error: '
               ...
               ELSE
                  IF    l_offr_lock = 0
                     OR lock_offr_chk (offr_sls.offr_id, l_offr_lock_user) <>
                           0
                  THEN
                     l_result := 1;

                     BEGIN
                        save_edit_offr_lines (offr_sls.offr_id,
                                              offr_sls.sls_typ,
                                              p_data_line);

                        --manage_scenario(offr_sls.offr_id, l_offr_lock_user, l_scnrio_offrs);

                        IF l_scnrio_offrs.COUNT > 0
                        THEN
                           FOR offr_idx IN l_scnrio_offrs.FIRST ..
                                           l_scnrio_offrs.LAST
                           LOOP
                              p_result.EXTEND;
                              p_result (p_result.LAST) :=
                                 obj_edit_offr_save_line (
                                    l_result,
                                    l_scnrio_offrs (offr_idx).p_offr_id,
                                    g_sls_typ_id);
                           END LOOP;
                        END IF;
            ...
            p_result.EXTEND ();
            p_result (p_result.LAST) :=
               obj_edit_offr_save_line (l_result,
                                        offr_sls.offr_id,
                                        offr_sls.sls_typ);
   ...
   EXCEPTION
      ...
      THEN
         app_plsql_log.info (
            l_module_name || ' Error in save edit offr' || SQLERRM (SQLCODE));
      ...
      IS
           SELECT o.*
             FROM TABLE (p_get_offr) poi
                  JOIN edit_offr_hist o
                     ON     o.intrnl_offr_id = poi.p_offr_id
                        AND o.sls_typ = poi.p_sls_typ
         ORDER BY o.hist_ts DESC, o.offr_sku_line_id;
   BEGIN
      FOR offer IN c_offr
      LOOP
         PIPE ROW (obj_pedit_offr_hist_line (offer.hist_ts,
                                            ...
                                            offer.pymt_typ,
                                            offer.promtn_id,
                                            offer.promtn_clm_id-- ,offer.cmbntn_offr_typ
                                            ,
                                            offer.spndng_lvl,
                                            offer.comsn_typ,
                                            offer.tax_type_id,
   ...
   EXCEPTION
      ...
      THEN
         app_plsql_log.info (
            'Error in getting offer history ' || SQLERRM (SQLCODE));
   END get_history;
   ...
   BEGIN
      ...
      SELECT MAX (o.mrkt_id)
        INTO l_mrkt_id
        FROM TABLE (p_get_offr) poi JOIN offr o ON o.offr_id = poi.p_offr_id;
      ...
      SELECT DISTINCT (o.veh_id)
        BULK COLLECT INTO l_veh_ids
        FROM TABLE (p_get_offr) poi JOIN offr o ON o.offr_id = poi.p_offr_id;

      SELECT MIN (NVL (p_sls_typ, -1))
        INTO l_min_sls_typ
        FROM TABLE (p_get_offr);
      ...
      IF    l_min_sls_typ <> l_sls_typ
         ...
         OR l_min_offr_perd_id <> l_offr_perd_id
      THEN
      ...
      DBMS_OUTPUT.put_line (
         l_sls_typ || l_mrkt_id || l_ver_id || l_offr_perd_id);

      FOR rec
         IN (SELECT NULL AS intrnl_offr_id--,l_sls_typ AS sls_typ
                    ,
                    NVL (
                       osl_current.sales_type,
                       ...
                       AS sls_typ--latest version calculations
                    ,
                    NULL AS lv_nta,
                    NULL AS lv_sp,
                    NULL AS lv_rp,
                    ...
                    NULL lv_dp_cash,
                    NULL lv_dp_percent--/latest version calculations
                    ,
                    0 AS status,
                    ...
                    NULL AS pymt_typ,
                    NULL AS promtn_id,
                    NULL AS promtn_clm_id--,1 AS cmbntn_offr_typ
                    ,
                    ...
                    NULL AS offr_sku_set_nm--pricing corridors
                    ,
                    NULL AS pc_sp_py,
                    NULL AS pc_rp,
                    NULL AS pc_sp,
               ...
               FROM (SELECT *
                       FROM offr
                      WHERE     offr_id IN (SELECT p_offr_id
                                              FROM TABLE (p_get_offr))
                    ...
                    (SELECT offr_sku_line.offr_sku_line_id,
                            ...
                            CASE
                               WHEN actual_sku.wghtd_avg_cost_amt IS NOT NULL
                               THEN
                            ...
                            sku.sku_nm--,l_sls_typ AS sales_type
                            ,
                            CASE
                               WHEN l_sls_typ IS NULL
                               THEN
                                  (SELECT MAX (mps_sls_typ_id)
                                     FROM mrkt_veh_perd_ver mvpv
                                    WHERE     mvpv.mrkt_id = l_mrkt_id
                            ...
                            (  SELECT NVL (SUM (unit_qty), 0) AS sum_unit_qty
                                ...
                                WHERE     sls_typ_id =             --l_sls_typ
                                      ...
                                      AND offr_perd_id = l_offr_perd_id
                                      AND sls_perd_id = l_offr_perd_id
                                      AND offr_sku_line_id =
                                             offr_sku_line.offr_sku_line_id
                             GROUP BY offr_sku_line_id, sls_typ_id)
                               sum_unit_qty,
                      ...
                      WHERE     o.offr_id = offr_sku_line.offr_id
                            ...
                            AND o.offr_id IN (SELECT p_offr_id
                                                FROM TABLE (p_get_offr))
                            ...
                            AND sku.sku_id(+) = offr_sku_line.sku_id)
                    osl_current,
                    offr_prfl_prc_point,
                    prfl,
                    form,
                    brnd_grp,
                    brnd
              WHERE     --offr outer join on selected version
      ...
      LOOP
         PIPE ROW (obj_pedit_offr_line (rec.status,
                                       rec.mrkt_id,
                                       rec.offr_perd_id,
                                       rec.offr_lock,
                                       rec.offr_lock_user,
   ...
   AS
      ...
      IS
           SELECT *
             FROM TABLE (p_filters)
         ORDER BY p_mrkt_id, p_offr_perd_id;
      ...
      l_OBJ_PEDIT_OFFR_TABLE   OBJ_PEDIT_OFFR_TABLE;
      l_pagination            CHAR (1);
      l_scnrio_cnt            NUMBER;
   ...
   BEGIN
      ...
      FOR p_filter IN c_p_filter
      LOOP                                      --Filters from the screen loop
         IF p_filter.p_scnrio_id IS NOT NULL
         THEN
         ...
         FOR offrs
            IN (--Return every offer id where not every item is disabled.
                WITH mrkt_tmp_fsc_master
                     AS (SELECT mrkt_id, sku_id, fsc_cd
                           FROM (SELECT mrkt_id,
                                        sku_id,
                                        mstr_fsc_cd fsc_cd,
                                        ...
                                        NVL (
                                           LEAD (
                                              strt_perd_id,
                                              1)
                                           OVER (PARTITION BY mrkt_id, sku_id
                                                 ORDER BY strt_perd_id),
                                           99999999)
                                           to_strt_perd_id
                                   FROM mstr_fsc_asgnmt
                                  WHERE     p_filter.p_mrkt_id = mrkt_id
                                        AND p_filter.p_offr_perd_id >=
                                               strt_perd_id)
                          WHERE     p_filter.p_offr_perd_id >=
                                       from_strt_perd_id
                                AND p_filter.p_offr_perd_id < to_strt_perd_id),
                     mrkt_tmp_fsc
                     AS (  SELECT mrkt_id, sku_id, MAX (fsc_cd) fsc_cd
                             FROM (SELECT mrkt_id,
                                          sku_id,
                                          fsc_cd fsc_cd,
                                          strt_perd_id from_strt_perd_id,
                                          NVL (
                                             LEAD (
                                                strt_perd_id,
                                                1)
                                             OVER (PARTITION BY mrkt_id, fsc_cd
                                                   ORDER BY strt_perd_id),
                                             99999999)
                                             to_strt_perd_id
                                     FROM mrkt_fsc
                  ...
                  SELECT o.offr_id AS p_offr_id,
                   ...
                   WHERE                                  --offr_sku_line join
                        o    .offr_id = osl.offr_id(+)
                         AND CASE
                                WHEN     p_filter.p_offr_typ IS NULL
                                     AND l_scnrio_cnt = 0
                                THEN
                                   1
                                WHEN     p_filter.p_offr_typ IS NULL
                                ...
                                THEN
                                   1
                                WHEN     p_filter.p_offr_typ IS NULL
                                     AND what_if_tran.scnrio_id IN (SELECT *
                                                                      FROM TABLE (
                                                                              p_filter.p_scnrio_id))
                                THEN
                                   1
                                WHEN     p_filter.p_offr_typ IN ('CMP', 'WIF')
                                     AND o.offr_typ = p_filter.p_offr_typ
                                THEN
                                   1
                                ELSE
                                   0
                         ...
                         AND dstrbtd_mrkt_sls.sls_typ_id(+) =
                                p_filter.p_sls_typ
                         AND CASE
                                WHEN p_filter.p_offr_id IS NULL
                                THEN
                                   1
                                WHEN o.offr_id IN (SELECT *
                                                     FROM TABLE (
                                                             p_filter.p_offr_id))
                                THEN
                                   1
                             ...
                             END = 1
                         AND CASE
                                WHEN p_filter.p_veh_id IS NULL
                                THEN
                                   1
                                WHEN o.veh_id IN (SELECT *
                                                    FROM TABLE (
                                                            p_filter.p_veh_id))
                                THEN
                                   1
                             ...
                             END = 1
                         AND CASE
                                WHEN p_filter.p_brchr_plcmt_id IS NULL
                                THEN
                                   1
                                WHEN o.brchr_plcmt_id IN (SELECT *
                                                            FROM TABLE (
                                                                    p_filter.p_brchr_plcmt_id))
                                THEN
                                   1
                                ELSE
                                   0
                             END = 1
                         AND CASE
                                WHEN p_filter.p_catgry_id IS NULL
                                THEN
                                   1
                                WHEN prfl.catgry_id IN (SELECT *
                                                          FROM TABLE (
                                                                  p_filter.p_catgry_id))
                                THEN
                                   1
                                ELSE
                                   0
                             END = 1
                         AND CASE
                                WHEN p_filter.p_brnd_id IS NULL
                                THEN
                                   1
                                WHEN brnd_grp.brnd_fmly_id IN (SELECT *
                                                                 FROM TABLE (
                                                                         p_filter.p_brnd_id))
                                THEN
                                   1
                             ...
                             END = 1
                         AND CASE
                                WHEN p_filter.p_prfl_cd IS NULL
                                THEN
                                   1
                                WHEN offr_prfl_prc_point.prfl_cd IN (SELECT *
                                                                       FROM TABLE (
                                                                               p_filter.p_prfl_cd))
                                THEN
                                   1
                             ...
                             END = 1
                         AND CASE
                                WHEN p_filter.p_sku_id IS NULL
                                THEN
                                   1
                                WHEN osl.sku_id IN (SELECT *
                                                      FROM TABLE (
                                                              p_filter.p_sku_id))
                                THEN
                                   1
                             ...
                             END = 1
                         AND CASE
                                WHEN p_filter.p_fsc_cd IS NULL
                                THEN
                                   1
                                WHEN NVL (mrkt_tmp_fsc_master.fsc_cd,
                                          mrkt_tmp_fsc.fsc_cd) IN (SELECT *
                                                                     FROM TABLE (
                                                                             p_filter.p_fsc_cd))
                                THEN
                                   1
                             ...
                             END = 1
                         AND CASE
                                WHEN p_filter.p_enrgy_chrt_postn_id IS NULL
                                THEN
                                   1
                                WHEN o.enrgy_chrt_postn_id IN (SELECT *
                                                                 FROM TABLE (
                                                                         p_filter.p_enrgy_chrt_postn_id))
                                THEN
                                   1
                                ELSE
                                   0
                             END = 1
                         AND CASE
                                WHEN p_filter.p_web_postn_id IS NULL
                                THEN
                                   1
                                WHEN o.web_postn_id IN (SELECT *
                                                          FROM TABLE (
                                                                  p_filter.p_web_postn_id))
                                THEN
                                   1
                                ELSE
                                   0
         ...
         LOOP                                      --get valid offer id-s loop
            ...
            l_get_offr_table (l_get_offr_table.LAST) :=
               obj_get_offr_line (offrs.p_offr_id, NVL (offrs.p_sls_typ, 1));
            ...
            FOR rec
               IN (  SELECT o.*
                   ...
                   ORDER BY mv.seq_nr,
                            ...
                            o.sku_nm)
            LOOP                                                 --Result loop
               l_OBJ_PEDIT_OFFR_TABLE.EXTEND ();
               l_OBJ_PEDIT_OFFR_TABLE (l_OBJ_PEDIT_OFFR_TABLE.LAST) :=
                  obj_pedit_offr_line (rec.status,
                                      rec.mrkt_id,
                                      rec.offr_perd_id,
                                      rec.offr_lock,
                                      rec.offr_lock_user,
         ...
         ELSE
            FOR rec IN (SELECT o.*
                          FROM TABLE (get_offr_pivot (l_get_offr_table)) o)
            LOOP                                                 --Result loop
               l_OBJ_PEDIT_OFFR_TABLE.EXTEND ();
               l_OBJ_PEDIT_OFFR_TABLE (l_OBJ_PEDIT_OFFR_TABLE.LAST) :=
                  obj_pedit_offr_line (rec.status,
      ...
      BEGIN
         merge_history (l_OBJ_PEDIT_OFFR_TABLE);
      END;
   EXCEPTION
      WHEN NO_DATA_NEEDED
      THEN
   ...
   BEGIN
      ...
      BEGIN
         ...
         SELECT mrkt_config_item_val_txt
          ...
          WHERE mrkt_id = l_mrkt_id AND config_item_id = cfg_pricing_market;
      ...
      FOR rec
         IN (WITH latest_version_and_sls_typ
                  AS (SELECT asl.ver_id,
                             asl.sls_typ_id,
                             asl.offr_perd_id,
                             asl.veh_id
                        FROM (  SELECT ver_id,
                                  ...
                                  FROM (SELECT ver_id,
                                          ...
                                          FROM (  SELECT dstrbtd_mrkt_sls.ver_id,
                                                ...
                                                GROUP BY dstrbtd_mrkt_sls.ver_id,
                                                         ...
                                                         dstrbtd_mrkt_sls.offr_perd_id,
                                                         dstrbtd_mrkt_sls.veh_id))
                                 WHERE max_seq_nr = seq_nr AND seq_nr <> 100
                              GROUP BY ver_id, offr_perd_id, veh_id) asl),
                  latest_osl_stuff
                  AS (  SELECT osl.sku_id,
                               osl.offr_prfl_prcpt_id,
                               o.mrkt_id,
                               o.offr_perd_id,
                          ...
                          FROM offr o
                               ...
                               JOIN latest_version_and_sls_typ
                                  ON     o.offr_perd_id =
                                     ...
                                     AND o.veh_id =
                                            latest_version_and_sls_typ.veh_id
                                     AND o.ver_id =
                                            latest_version_and_sls_typ.ver_id
                                     AND o.offr_perd_id =
                                            latest_version_and_sls_typ.offr_perd_id
                                     AND dms.sls_typ_id =
                  ...
                  AS (  SELECT mrkt_id, sku_id, MAX (fsc_cd) fsc_cd
                      ...
                      GROUP BY mrkt_id, sku_id),
                  frcst
                  AS (SELECT ML_WHAT_IF_SCNRIO_OSL.offr_sku_line_id,
                             offr_sku_line.offr_prfl_prcpt_id,
                             ML_WHAT_IF_SCNRIO_OSL.ML_UNIT_QTY
                                forcasted_units,
                             ML_WHAT_IF_SCNRIO_OSL.last_updt_ts
                                forcasted_date
                        FROM offr_sku_line, ML_WHAT_IF_SCNRIO_OSL
                       WHERE     offr_sku_line.mrkt_id =
                                    ML_WHAT_IF_SCNRIO_OSL.mrkt_id
                             AND offr_sku_line.offr_perd_id =
                                    ML_WHAT_IF_SCNRIO_OSL.offr_perd_id
                             AND offr_sku_line.offr_sku_line_id =
                                    ML_WHAT_IF_SCNRIO_OSL.offr_sku_line_id
                             AND ML_WHAT_IF_SCNRIO_OSL.scnrio_id =
                                    p_scnrio_id)
             --
             SELECT o.offr_id AS intrnl_offr_id--,l_sls_typ AS sls_typ
                    ,
                    NVL (
                       osl_current.sales_type,
                       (SELECT MAX (mps_sls_typ_id)
                    ...
                    CASE
                       WHEN osl_latest.reg_prc_amt = 0
                       THEN
                          0
                       WHEN osl_latest.offr_prfl_sls_prc_amt = 0
                       THEN
                          100
                       ELSE
                          ROUND (
                             ...
                             -   (  osl_latest.offr_prfl_sls_prc_amt
                                  ...
                                  / osl_latest.reg_prc_amt)
                               * 100)
                    END
                       lv_discount,
                    osl_latest.unit_qty AS lv_units,
                    osl_latest.unit_qty * osl_latest.sku_cost_amt
                       AS lv_total_cost,
                    CASE
                    ...
                    CASE
                       WHEN    osl_latest.nr_for_qty = 0
                            ...
                            OR osl_latest.offr_prfl_sls_prc_amt = 0
                       THEN
                          0
                       ELSE
                            ROUND (
                          ...
                          * 100
                    END
                       lv_dp_percent--/latest version calculations
                    ,
                    0 AS status,
                    o.mrkt_id AS mrkt_id,
                    o.offr_perd_id AS offr_perd_id,
                    ...
                    offr_prfl_prc_point.prfl_cd AS prfl_cd,
                    osl_current.sku_id AS sku_id,
                    NVL (mrkt_tmp_fsc_master.fsc_cd, mrkt_tmp_fsc.fsc_cd)
                       AS fsc_cd,
                    ...
                    offr_prfl_prc_point.cnsmr_invstmt_bdgt_id
                       AS cnsmr_invstmt_bdgt_id,
                    offr_prfl_prc_point.pymt_typ AS pymt_typ,
                    offr_prfl_prc_point.promtn_id AS promtn_id,
                    offr_prfl_prc_point.promtn_clm_id AS promtn_clm_id--,1 AS cmbntn_offr_typ
                    ,
                    'NEW_FEATURE' AS spndng_lvl,
                    ...
                    offr_prfl_prc_point.sls_prc_amt AS sls_prc_amt,
                    CASE
                       WHEN osl_current.offr_sku_line_id IS NOT NULL
                       THEN
                          NVL (osl_current.reg_prc_amt, 0)
                       ELSE
                          NULL
                    END
                       AS reg_prc_amt                         --sku reg prcb?l
                                     ,
                    ...
                    NVL (os.log_user_id, o.last_updt_user_id)
                       AS last_updt_user_id,
                    NVL (os.log_ts, o.last_updt_ts) AS last_updt_ts,
                    mrkt_veh_perd_sctn.mrkt_veh_perd_sctn_id
                       AS mrkt_veh_perd_sctn_id,
                    prfl.prfl_nm AS prfl_nm,
                    ...
                    offr_sku_set.offr_sku_set_nm AS offr_sku_set_nm--pricing corridors
                    ,
                    CASE
                       WHEN p_pagination = 'N'
                       THEN
                    ...
                    CASE
                       WHEN offr_prfl_prc_point.featrd_side_cd = 1
                       THEN
                            (SELECT MAX (
                                       DECODE (
                                          o.mrkt_veh_perd_sctn_id,
                       ...
                       DECODE (
                          ...
                          0, CASE
                                WHEN SUM (
                                     ...
                                     OVER (
                                        PARTITION BY offr_prfl_prc_point.offr_prfl_prcpt_id,
                                                     osl_current.sales_type) =
                                        0
                                THEN
                                   AVG (
                                      osl_current.sku_cost_amt)
                                   OVER (
                                      PARTITION BY offr_prfl_prc_point.offr_prfl_prcpt_id,
                                                   osl_current.sum_unit_qty)
                                ELSE
                          ...
                          CASE
                             WHEN SUM (
                                  ...
                                  OVER (
                                     PARTITION BY offr_prfl_prc_point.offr_prfl_prcpt_id,
                                                  osl_current.sales_type) = 0
                             THEN
                                AVG (
                                   osl_current.sum_cost_amt)
                                OVER (
                                   PARTITION BY offr_prfl_prc_point.offr_prfl_prcpt_id,
                                                osl_current.sales_type)
                             ELSE
               ...
               FROM (SELECT *
                    ...
                    mrkt_tmp_fsc_master,
                    frcst,
                    (SELECT mpsp.rp AS pc_rp,
                            mpsp.sp AS pc_sp,
                            mpsp.vsp AS pc_vsp,
                            mpsp.hit AS pc_hit,
                            mpsp.sku_id AS sku_id
                       FROM (SELECT mps.*
                               FROM (SELECT mrkt_perd_sku_prc.mrkt_id,
                                            mrkt_perd_sku_prc.offr_perd_id,
                                            mrkt_perd_sku_prc.sku_id,
                                            mrkt_perd_sku_prc.prc_lvl_typ_cd,
                                            mrkt_perd_sku_prc.crncy_cd,
                                            mrkt_perd_sku_prc.sku_prc_amt,
                                            mrkt_perd_sku_prc.cmpnt_discnt_pct
                                       FROM mrkt_perd_sku_prc,
                                            mrkt_prc_lvl mpl
                                      WHERE     mrkt_perd_sku_prc.mrkt_id =
                                                   l_mrkt_id
                                            AND mrkt_perd_sku_prc.offr_perd_id =
                                                   l_offr_perd_id
                                            AND mrkt_perd_sku_prc.sku_id IN (SELECT s.sku_id
                                                                               FROM sku s,
                                                                                    sku_reg_prc srp,
                                                                                    mrkt_sku ms
                                                                              WHERE     ms.mrkt_id =
                                                                                    ...
                                                                                    AND srp.mrkt_id =
                                                                                           l_mrkt_id
                                                                                    AND srp.offr_perd_id =
                                                                                           l_offr_perd_id
                                                                                    AND srp.sku_id =
                                                                                           s.sku_id)
                                            AND mpl.mrkt_id(+) =
                                                   mrkt_perd_sku_prc.mrkt_id
                                            AND mpl.prc_lvl_typ_cd(+) =
                                                   mrkt_perd_sku_prc.prc_lvl_typ_cd
                                            AND (   mpl.strt_perd_id IS NULL
                                                 OR mpl.strt_perd_id <=
                                                       l_offr_perd_id)
                                            AND (   mpl.end_perd_id IS NULL
                                                 OR mpl.end_perd_id >=
                                                       l_offr_perd_id)
                                            AND p_pagination = 'N') PIVOT (MAX (
                                                                              sku_prc_amt)
                                                                    FOR prc_lvl_typ_cd
                                                                    IN  ('RP' AS rp,
                                                                        'HIT' AS hit,
              ...
              WHERE     --mrkt_tmp_fsc and master
                    ...
                    AND offr_sls_cls_sku.sls_cls_cd(+) =
                           osl_current.sls_cls_cd
                    ...
                    --frcst
                    AND frcst.offr_sku_line_id(+) =
                           osl_current.offr_sku_line_id
                    AND frcst.offr_prfl_prcpt_id(+) =
                           osl_current.offr_prfl_prcpt_id
                    --pricing
   ...
   BEGIN
      FOR mp IN (SELECT * FROM TABLE (p_mrkt_perd))
      LOOP
         --market period loop
         FOR veh
            IN (SELECT veh_id, mps_sls_typ_id
                  FROM mrkt_veh_perd
                 WHERE     mrkt_id = mp.mrkt_id
                       AND offr_perd_id = mp.offr_perd_id)
         LOOP
            --veh_id loop
            BEGIN
            ...
            EXCEPTION
               ...
               THEN
                  app_plsql_log.info (
                        'Error in get_sls_typ_grp function: '
                     || SQLERRM (SQLCODE));
            END;
   ...
   BEGIN
      SELECT mci.mrkt_config_item_val_txt
       ...
       WHERE     mci.config_item_id = p_config_item_id
          ...
          SELECT TRIM (REGEXP_SUBSTR (l_conf_item_val_txt,
                                      co_pattern,
                                      1,
                                      LEVEL))
            BULK COLLECT INTO l_str_array
            FROM DUAL
      CONNECT BY REGEXP_SUBSTR (l_conf_item_val_txt,
                                co_pattern,
                                1,
                                LEVEL)
                    IS NOT NULL;
   ...
   FUNCTION query_default_values (p_mrkt_id IN NUMBER)
      RETURN t_default_values
   IS
   ...
   IS
      l_offr_table        obj_get_offr_table := obj_get_offr_table ();
      l_edit_offr_table   OBJ_PEDIT_OFFR_TABLE;
   BEGIN
      l_offr_table.EXTEND;
      l_offr_table (l_offr_table.LAST) :=
         obj_get_offr_line (p_offr_id, NVL (p_sls_typ_id, 1));
      ...
      SELECT obj_pedit_offr_line (status,
                                 ...
                                 offr_link_id,
                                 profile_item_count)
        BULK COLLECT INTO l_edit_offr_table
   ...
   BEGIN
      ...
      FOR sku_rec
         IN (SELECT ROWNUM,
                    pa_maps_public.get_sls_cls_cd (p_offr_perd_id,
                                                   p_mrkt_id,
                                                   s.sku_id)
                       sls_cls_cd,
                    s.sku_id,
                    srp.reg_prc_amt
               FROM sku s, sku_reg_prc srp
      ...
      LOOP
            ...
            THEN
               ...
               INSERT INTO offr_prfl_sls_cls_plcmt (offr_id,
                    ...
                    VALUES (p_offr_id,
                            sku_rec.sls_cls_cd,
                            p_prfl_cd,
                            l_default_values.pg_ofs_nr,
                            l_default_values.featrd_side_cd,
                            p_mrkt_id,
                            p_veh_id,
                            p_offr_perd_id,
                            0,
                            l_default_values.pg_wght_pct,
                            l_default_values.prod_endrsmt_id,
                            l_default_values.pg_typ_id,
                            p_user_nm);
         END;
         ...
         BEGIN
            ...
            BEGIN
               l_tax_type_id :=
                  pa_maps_gta.get_default_tax_type_id (p_mrkt_id,
                                                       p_prfl_cd,
                                                       sku_rec.sls_cls_cd,
                                                       p_offr_perd_id,
                                                       p_veh_id);
               l_tax_pct :=
                  get_tax_rate (p_mrkt_id, l_tax_type_id, p_offr_perd_id);
            EXCEPTION
               WHEN OTHERS
               THEN
                  l_tax_pct := 0;
            ...
            BEGIN
               l_comsn_typ :=
                  pa_maps_gta.get_commission_type (p_mrkt_id,
                                                   p_veh_id,
                                                   p_offr_perd_id,
                                                   p_prfl_cd,
                                                   'N',
                                                   ...
                                                   NULL);
               l_comsn_pct :=
                  get_comsn_pct (p_mrkt_id, p_offr_perd_id, l_comsn_typ);
            EXCEPTION
               WHEN OTHERS
               THEN
                  l_comsn_pct := 0;
         ...
         EXCEPTION
            ...
            THEN
               ...
               SELECT crncy_cd
                 INTO l_crncy_cd
                 FROM mrkt_perd
                WHERE mrkt_id = p_mrkt_id AND perd_id = p_offr_perd_id;

               -- calculate GTA
               l_location := 'calculate GTA';

               BEGIN
                  SELECT NVL (gta_mthd_id, 1)
                    INTO l_gta_mthd_id
                    FROM mrkt_perd
                   WHERE mrkt_id = p_mrkt_id AND perd_id = p_offr_perd_id;
                  ...
                  l_net_to_avon_fct :=
                     pa_maps_gta.get_gta_without_price_point (
                        l_gta_mthd_id,
                        l_sls_prc_amt,
                        l_default_values.chrty_amt,
                        l_default_values.awrd_sls_prc_amt,
                        l_comsn_pct,
                        l_tax_pct,
                        0);
               EXCEPTION
               ...
               INSERT INTO offr_prfl_prc_point (offr_prfl_prcpt_id,
                                                offr_id,
                                                promtn_clm_id,
                                                veh_id,
                                                promtn_id,
                                                ...
                                                tax_type_id,
                                                creat_user_id)
                    VALUES (l_offr_prfl_prcpt_id,
                            p_offr_id,
                            l_default_values.promtn_clm_id,
                            p_veh_id,
                            l_default_values.promtn_id,
                            p_mrkt_id,
                            l_cnsmr_invstmt_bdgt_id,
                            sku_rec.sls_cls_cd,
                            p_prfl_cd,
                            l_default_values.ssnl_evnt_id,
                            p_offr_perd_id,
                            l_crncy_cd,
                            0,
                            l_default_values.nr_for_qty,
                            l_default_values.unit_qty,
                            l_sls_prc_amt,
                            l_default_values.wghtd_avg_cost_amt,
                            l_default_values.sls_srce_id,
                            l_sls_prc_amt,
                            l_tax_pct,
                            l_pymt_typ,
                            l_comsn_pct,
                            l_comsn_typ,
                            l_net_to_avon_fct,
                            l_default_values.prmry_offr_ind,
                            l_default_values.pg_ofs_nr,
                            l_default_values.featrd_side_cd,
                            l_default_values.chrty_amt,
                            l_default_values.awrd_sls_prc_amt,
         ...
         INSERT INTO offr_sku_line (offr_sku_line_id,
                                    offr_id,
                                    veh_id,
                                    featrd_side_cd,
                                    offr_perd_id,
         ...
         IF g_sls_typ_id IN (co_sls_typ_estimate, co_sls_typ_op_estimate)
         THEN
         ...
         IF g_sls_typ_id IN (co_sls_typ_op_estimate)
         THEN
         ...
         UPDATE offr
            SET sku_cnt = NVL (sku_cnt, 0) + 1, last_updt_user_id = p_user_nm
          WHERE offr_id = p_offr_id;
   ...
   BEGIN
      ...
      BEGIN
         l_tax_amt :=
            get_tax_rate (p_mrkt_id,
                          l_default_values.tax_type_id,
                          p_offr_perd_id);
      EXCEPTION
         WHEN OTHERS
         THEN
      ...
      BEGIN
         l_comsn_amt :=
            get_comsn_pct (p_mrkt_id,
                           p_offr_perd_id,
                           l_default_values.comsn_typ);
      EXCEPTION
         WHEN OTHERS
         THEN
            l_comsn_amt := 0;
      ...
      l_net_to_avon_fct :=
         pa_maps_gta.get_gta_without_price_point (
            pa_maps_gta.pri_get_gta_method_id (p_mrkt_id, p_offr_perd_id),
            p_pp_rec.sls_prc_amt,
            l_default_values.chrty_amt,
            l_default_values.awrd_sls_prc_amt,
            l_comsn_amt,
            l_tax_amt,
      ...
      INSERT INTO offr_prfl_prc_point (offr_prfl_prcpt_id,
           ...
           VALUES (l_offr_prfl_prcpt_id,
                   ...
                   l_default_values.promtn_id,
                   p_mrkt_id,
                   p_pp_rec.cnsmr_invstmt_bdgt_id,
                   p_pp_rec.sls_cls_cd,
                   p_pp_rec.prfl_cd,
                   l_default_values.ssnl_evnt_id,
                   p_offr_perd_id,
                   ...
                   l_default_values.nr_for_qty,
                   l_default_values.unit_qty,
                   p_pp_rec.est_sls_amt,
                   l_default_values.wghtd_avg_cost_amt,
                   l_default_values.sls_srce_id,
                   0,
                   l_tax_amt,
                   l_default_values.pymt_typ,
                   l_comsn_amt,
                   l_default_values.comsn_typ,
                   l_net_to_avon_fct,
                   l_default_values.prmry_offr_ind,
      ...
      UPDATE offr
         SET prfl_cnt = NVL (prfl_cnt, 0) + 1, last_updt_user_id = p_user_nm
       WHERE offr_id = p_offr_id;

      FOR sku_rec
         IN (SELECT osl.*, srp.reg_prc_amt
               FROM offr_sku_line osl, sku_reg_prc srp
   ...
   EXCEPTION
      ...
      THEN
         app_plsql_log.info (
            l_procedure_name || ': Error adding pricepoint at ' || l_location);
         app_plsql_log.info (l_procedure_name || ': ' || SQLERRM (SQLCODE));
         p_status := co_exec_status_failed;
         RAISE;
   END add_pricepoint;
   ...
   BEGIN
      ...
      BEGIN
         SELECT mv.dfalt_offr_cls_id
           INTO l_offr_cls_id
           FROM mrkt_veh mv
          WHERE mv.mrkt_id = p_mrkt_id AND mv.veh_id = p_veh_id;

         IF l_offr_cls_id IS NULL
         THEN
      ...
      INSERT INTO offr (offr_id,
           ...
           VALUES (l_offr_id,
                   ...
                   NVL (p_offr_typ, l_default_values.offr_typ),
                   l_brchr_plcmt_id,
                   p_sctn_page_ofs_nr,
                   0,
                   0,
                   p_featrd_side_cd,
                   l_default_values.flap_ind,
                   l_default_values.offr_stus_cd,
                   p_offr_perd_id,
                   p_offr_perd_id,
                   l_default_values.brchr_postn_id,
                   l_default_values.unit_rptg_lvl_id,
                   l_default_values.rpt_sbtl_typ_id,
                   l_default_values.pg_typ_id,
                   l_offr_cls_id,
                   p_user_nm,
                   'N');

      IF p_prfl_cd_list IS NOT NULL AND p_prfl_cd_list.COUNT > 0
      THEN
         l_location := 'Calling add_concept';

         FOR i IN p_prfl_cd_list.FIRST .. p_prfl_cd_list.LAST
         LOOP
      ...
      l_get_offr_table.EXTEND ();
      l_get_offr_table (l_get_offr_table.LAST) :=
         obj_get_offr_line (l_offr_id, 1);                           --KK null
   ...
   EXCEPTION
      ...
      THEN
         p_status := co_exec_status_failed;
         app_plsql_log.info (
            l_procedure_name || ': Error adding offer at ' || l_location);
         app_plsql_log.info (l_procedure_name || ': ' || SQLERRM (SQLCODE));
   ...
   BEGIN
      ...
      LOOP
         ...
         IF p_status = co_exec_status_prcpnt_ex
         THEN
            ROLLBACK;
            app_plsql_log.info (
                  l_procedure_name
               || ': Pricepoint with default values already exists. offr_prfl_prcpt_id: '
               || p_prfl_cd_list (i));
      ...
      l_location := 'getting offer table';
      p_edit_offr_table :=
         get_offr_table (p_offr_id => p_offr_id, p_pagination => p_pagination);
   ...
   EXCEPTION
      ...
      THEN
         ...
         app_plsql_log.info (
               l_procedure_name
            || ': Error adding concepts to offer at '
            || l_location);
         app_plsql_log.info (l_procedure_name || ': ' || SQLERRM (SQLCODE));
   ...
   BEGIN
      ...
      SELECT o.mrkt_id, o.offr_perd_id, o.veh_id
        INTO l_mrkt_id, l_offr_perd_id, l_veh_id
        FROM offr o
       WHERE o.offr_id = p_offr_id;
      ...
      FOR prpct_rec
         IN (SELECT p.*
              ...
              WHERE p.offr_prfl_prcpt_id IN (SELECT COLUMN_VALUE
                                               FROM TABLE (
                                                       p_offr_prfl_prcpt_id_list)))
      LOOP
   ...
   EXCEPTION
      ...
      THEN
         ...
         app_plsql_log.info (
               l_procedure_name
            || ': Error adding pricepoints to offer at '
            || l_location);
         app_plsql_log.info (l_procedure_name || ': ' || SQLERRM (SQLCODE));
   ...
   IS
      ...
      l_obj_copy_offr       obj_pcopy_offr_line;
      l_offr_table          obj_get_offr_table := obj_get_offr_table ();
   ...
   BEGIN
      ...
      FOR i IN p_copy_offr_table.FIRST .. p_copy_offr_table.LAST
      LOOP
         l_obj_copy_offr := p_copy_offr_table (i);

         FOR j IN l_obj_copy_offr.offr_id.FIRST ..
                  l_obj_copy_offr.offr_id.LAST
         LOOP
            ...
            IF l_obj_copy_offr.trg_offr_typ = 'WIF'
            THEN
               l_whatif := TRUE;
            END IF;
            ...
            l_new_offr_id :=
               pa_maps_copy.copy_offer (
                  par_offerid          => l_old_offr_id,
                  ...
                  par_zerounits        => CASE
                                            WHEN l_obj_copy_offr.trg_zerounits =
                                                    1
                                            THEN
                                               TRUE
                                            WHEN l_obj_copy_offr.trg_zerounits =
                  ...
                  par_whatif           => l_whatif,
                  par_enrgychrt        => CASE
                                            WHEN l_obj_copy_offr.trg_enrgychrt =
                                                    1
                                            THEN
                                               TRUE
                                            WHEN l_obj_copy_offr.trg_enrgychrt =
            ...
            THEN
               ...
               THEN
                  add_scenario (
                     p_mrkt_id           => l_obj_copy_offr.trg_mrkt_id,
      ...
      app_plsql_log.info (l_procedure_name || ' stop');
   EXCEPTION
      WHEN e_copy_offer_failed
      THEN
         p_status := co_exec_status_failed;
         app_plsql_log.info (
               l_procedure_name
            || ': Copying offer failed. offr_id: '
            || l_old_offr_id);
   ...
   BEGIN
      SELECT COUNT (*)
        INTO l_cnt
        FROM (SELECT t.offr_sku_line_id
   ...
   PROCEDURE del_offer_with_deps (p_offr_id IN offr.offr_id%TYPE)
   IS
   ...
   BEGIN
      ...
      DELETE FROM dstrbtd_mrkt_sls dms
            WHERE dms.offr_sku_line_id IN (SELECT offr_sku_line_id
                                             FROM offr_sku_line
                                            WHERE offr_prfl_prcpt_id IN (SELECT offr_prfl_prcpt_id
      ...
      DELETE FROM offr_sku_line osl
            WHERE osl.offr_prfl_prcpt_id IN (SELECT offr_prfl_prcpt_id
                                               FROM offr_prfl_prc_point
                                              WHERE offr_id = p_offr_id);
   ...
   PROCEDURE add_to_edit_offr_table (
      p_offr_id           IN            offr.offr_id%TYPE,
      p_status            IN            NUMBER,
      p_osl_records       IN            OBJ_PEDIT_OFFR_TABLE,
      p_edit_offr_table   IN OUT NOCOPY OBJ_PEDIT_OFFR_TABLE)
   IS
   BEGIN
      FOR osl_rec IN (SELECT *
                        FROM TABLE (p_osl_records)
                       WHERE intrnl_offr_id = p_offr_id)
      LOOP
         p_edit_offr_table.EXTEND;
         p_edit_offr_table (p_edit_offr_table.LAST) :=
            obj_pedit_offr_line (p_status,
                                osl_rec.mrkt_id,
                                osl_rec.offr_perd_id,
                                osl_rec.offr_lock,
                                osl_rec.offr_lock_user,
                                osl_rec.offr_sku_line_id,
                                osl_rec.veh_id,
                                osl_rec.brchr_plcmnt_id,
                                osl_rec.brchr_sctn_nm,
                                osl_rec.enrgy_chrt_postn_id,
                                osl_rec.web_postn_id,
                                osl_rec.pg_nr,
                                osl_rec.ctgry_id,
                                osl_rec.brnd_id,
                                osl_rec.sgmt_id,
                                ...
                                osl_rec.gender_id,
                                osl_rec.sls_cls_cd,
                                osl_rec.pp_sls_cls_cd,
                                osl_rec.item_sls_cls_cd,
                                osl_rec.offr_desc_txt,
                                osl_rec.offr_notes_txt,
                                osl_rec.offr_lyot_cmnts_txt,
                                osl_rec.featrd_side_cd,
                                osl_rec.concept_featrd_side_cd,
                                osl_rec.micr_ncpsltn_ind,
                                osl_rec.scntd_pg_typ_id,
                                osl_rec.cnsmr_invstmt_bdgt_id,
                                osl_rec.pymt_typ,
                                osl_rec.promtn_id,
                                osl_rec.promtn_clm_id,
                                osl_rec.spndng_lvl,
                                osl_rec.comsn_typ,
                                osl_rec.tax_type_id,
                                osl_rec.wsl_ind,
                                osl_rec.offr_sku_set_id,
                                osl_rec.cmpnt_qty,
                                osl_rec.nr_for_qty,
                                osl_rec.nta_factor,
                                osl_rec.sku_cost,
                                ...
                                osl_rec.lv_discount,
                                osl_rec.lv_units,
                                osl_rec.lv_total_cost,
                                osl_rec.lv_gross_sales,
                                osl_rec.lv_dp_cash,
                                osl_rec.lv_dp_percent,
                                osl_rec.ver_id,
                                osl_rec.sls_prc_amt,
                                osl_rec.reg_prc_amt,
                                osl_rec.line_nr,
                                ...
                                osl_rec.dltd_ind,
                                osl_rec.created_ts,
                                osl_rec.created_user_id,
                                osl_rec.last_updt_ts,
                                osl_rec.last_updt_user_id,
                                osl_rec.intrnl_offr_id,
                                osl_rec.mrkt_veh_perd_sctn_id,
                                osl_rec.prfl_nm,
                                osl_rec.sku_nm,
                                osl_rec.comsn_typ_desc_txt,
                                osl_rec.tax_typ_desc_txt,
                                osl_rec.offr_sku_set_nm,
                                osl_rec.sls_typ,
                                osl_rec.pc_sp_py,
                                osl_rec.pc_rp,
                                osl_rec.pc_sp,
                                ...
                                osl_rec.pp_pg_wght,
                                osl_rec.sprd_nr,
                                osl_rec.offr_prfl_prcpt_id,
                                osl_rec.has_unit_qty,
                                osl_rec.offr_typ,
                                osl_rec.forcasted_units,
                                osl_rec.forcasted_date,
                                osl_rec.offr_cls_id,
                                osl_rec.spcl_ordr_ind,
                                osl_rec.offr_ofs_nr,
                                osl_rec.pp_ofs_nr,
                                osl_rec.impct_catgry_id,
                                osl_rec.hero_ind,
                                osl_rec.smplg_ind,
                                osl_rec.mltpl_ind,
                                osl_rec.cmltv_ind,
                                osl_rec.use_instrctns_ind,
                                osl_rec.pg_typ_id,
                                osl_rec.featrd_prfl_ind,
                                osl_rec.fxd_pg_wght_ind,
                                osl_rec.prod_endrsmt_id,
                                osl_rec.frc_mtch_mthd_id,
                                osl_rec.wghtd_avg_cost_amt,
                                osl_rec.incntv_id,
                                osl_rec.intrdctn_perd_id,
                                osl_rec.on_stus_perd_id,
                                osl_rec.dspostn_perd_id,
                                osl_rec.scnrio_id,
                                osl_rec.micr_ncpsltn_desc_txt,
                                osl_rec.offr_link_id,
                                osl_rec.profile_item_count);
      END LOOP;
   ...
   BEGIN
      ...
      p_edit_offr_table := OBJ_PEDIT_OFFR_TABLE ();

      l_location := 'offr_rec loop';

      FOR offr_rec
         IN (SELECT DISTINCT intrnl_offr_id offr_id, offr_lock_user
               FROM TABLE (p_osl_records))
      LOOP
         BEGIN
            ...
            IF NOT chk_sku_lines (l_offr_id, p_osl_records)
            THEN
               RAISE e_not_all_sku_lines;
            END IF;

            IF     offr_rec.offr_lock_user IS NOT NULL
               AND lock_offr_chk (l_offr_id, offr_rec.offr_lock_user) = 0
            THEN
               RAISE e_lock_failed;
            END IF;
         ...
         EXCEPTION
            ...
            THEN
               ...
               app_plsql_log.info (
                  ...
                  || l_offr_id);
               app_plsql_log.info (
                  l_procedure_name || ': ' || SQLERRM (SQLCODE));
         END;

         l_location := 'adding data to edit_offr_table';
         add_to_edit_offr_table (l_offr_id,
                                 l_status,
                                 p_osl_records,
                                 p_edit_offr_table);
      END LOOP;
   ...
   PROCEDURE del_prcpt_with_deps (
      p_prcpt_id   IN offr_prfl_prc_point.offr_prfl_prcpt_id%TYPE)
   IS
   ...
   BEGIN
      ...
      LOOP
         BEGIN
            IF     offr_rec.offr_lock_user IS NOT NULL
               AND lock_offr_chk (offr_rec.offr_id, offr_rec.offr_lock_user) =
                      0
            THEN
               RAISE e_lock_failed;
            END IF;
            ...
            FOR prcpt_rec
               IN (SELECT DISTINCT offr_prfl_prcpt_id
                     FROM TABLE (p_osl_records))
            LOOP
               BEGIN
            ...
            UPDATE offr o
               SET o.sku_cnt =
                      (SELECT COUNT (1)
                         FROM offr_sku_line osl, offr_prfl_prc_point oppp
                        WHERE     osl.offr_prfl_prcpt_id =
                                     oppp.offr_prfl_prcpt_id
         ...
         EXCEPTION
            ...
            THEN
               app_plsql_log.info (
                     l_procedure_name
                  || ': Error deleting pricepoints (lock failure) at '
                  || l_location
                  || ', offr_prfl_prcpt_id: '
                  || l_prcpt_id);
               app_plsql_log.info (
               ...
               l_lock_failed_prcpnts (l_lock_failed_prcpnts.LAST) :=
                  l_prcpt_id;
            WHEN OTHERS
            THEN
               app_plsql_log.info (
         ...
         l_get_offr_table.EXTEND;
         l_get_offr_table (l_get_offr_table.LAST) :=
            obj_get_offr_line (offr_rec.offr_id, 1);
      ...
      FOR i IN p_edit_offr_table.FIRST .. p_edit_offr_table.LAST
      LOOP
         SELECT COUNT (1)
           INTO l_cnt
           FROM DUAL
          WHERE p_edit_offr_table (i).offr_prfl_prcpt_id IN (SELECT COLUMN_VALUE
                                                               FROM TABLE (
                                                                       l_lock_failed_prcpnts));
   ...
   BEGIN
      ...
      INSERT INTO offr_prfl_prc_point (offr_prfl_prcpt_id,
           ...
           VALUES (l_offr_prfl_prcpt_id,
                   ...
                   p_pp_rec.sls_cls_cd,
                   p_pp_rec.prfl_cd,
                   p_pp_rec.ssnl_evnt_id,
                   l_offr_perd_id,
                   p_pp_rec.crncy_cd,
                   0,
                   p_pp_rec.nr_for_qty,
                   p_pp_rec.est_unit_qty,
                   p_pp_rec.est_sls_amt,
                   p_pp_rec.est_cost_amt,
                   p_pp_rec.sls_srce_id,
                   p_pp_rec.sls_prc_amt,
                   p_pp_rec.tax_amt,
                   p_pp_rec.pymt_typ,
                   p_pp_rec.comsn_amt,
                   p_pp_rec.comsn_typ,
                   p_pp_rec.net_to_avon_fct,
                   p_pp_rec.prmry_offr_ind,
                   p_trgt_pg_ofs_nr,
                   p_trgt_ftrd_side_cd,
                   p_pp_rec.chrty_amt,
                   p_pp_rec.awrd_sls_prc_amt,
                   p_pp_rec.tax_type_id,
                   p_user_nm);
      ...
      LOOP
            ...
            THEN
               ...
               INSERT INTO offr_sls_cls_sku (offr_id,
                    ...
                    VALUES (p_trgt_offr_id,
                            ...
                            l_smplg_ind,
                            l_hero_ind,
                            DECODE (l_scntd_pg_typ_id, NULL, 'N', 'Y'),
                            l_scntd_pg_typ_id,
                            l_wsl_ind,
                            l_reg_prc_amt,
                            l_cost_amt,
         ...
         INSERT INTO offr_sku_line (offr_sku_line_id,
              ...
              VALUES (l_offr_sku_line_id,
                      ...
                      p_pp_rec.prfl_cd,
                      sku_rec.crncy_cd,
                      sku_rec.prmry_sku_offr_ind,
                      sku_rec.sls_cls_cd,
                      l_offr_prfl_prcpt_id,
                      sku_rec.demo_avlbl_ind,
                      sku_rec.dltd_ind,
                      sku_rec.unit_splt_pct,
                      sku_rec.sls_prc_amt,
                      sku_rec.cost_typ,
                      p_user_nm);
   ...
   EXCEPTION
      ...
      THEN
         app_plsql_log.info (
               l_procedure_name
            || ': Pricepoint already exists, offr_prfl_prcpt_id: '
            || p_pp_rec.offr_prfl_prcpt_id);
         RAISE;
      WHEN OTHERS
      THEN
         app_plsql_log.info (
   ...
   BEGIN
      ...
      LOOP
         ...
         EXCEPTION
            ...
            THEN
               app_plsql_log.info (
                     l_procedure_name
                  || ': Error copying pricepoints at '
                  || l_location
                  || ', offr_prfl_prcpt_id: '
                  || r_pp_rec.offr_prfl_prcpt_id);
               app_plsql_log.info (
                  l_procedure_name || ': ' || SQLERRM (SQLCODE));
      ...
      l_location := 'getting offer table';
      p_edit_offr_table :=
         get_offr_table (p_offr_id => p_trgt_offr_id, p_pagination => 'Y');

      app_plsql_log.info (l_procedure_name || ' stop');
   EXCEPTION
      WHEN e_src_lock_failed
      THEN
         p_status := co_exec_status_failed;
         app_plsql_log.info (
               l_procedure_name
            || ': Source offer lock failed, user: '
            || p_user_nm
            || ', Status: '
            || l_lock_status);

         ROLLBACK;
      WHEN e_trgt_lock_failed
      THEN
         p_status := co_exec_status_failed;
         app_plsql_log.info (
               l_procedure_name
            || ': Target offer lock failed, user: '
            || p_user_nm
            || ', Status: '
            || l_lock_status);
