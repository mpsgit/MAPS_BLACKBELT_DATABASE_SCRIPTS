...
--  AND nvl(hist.cmbntn_offr_typ        ,-1)  = nvl(cur.cmbntn_offr_typ            ,-1)
  ...
  AND NVL(hist.offr_cls_desc_txt      ,-1)  = NVL(cur.offr_cls_desc_txt          ,-1)
   )
...
BEGIN
  ...
  LOOP
    app_plsql_log.info(SUBSTR(p_module_name || ', Params: ' ||
                  ...
                  ', offr_cls_desc_txt: ' ||             rec.offr_cls_desc_txt
                  , 1, 4000));
  END LOOP;
END log_params;
...
SELECT o.offr_id  AS intrnl_offr_id
      ...
      ,offr_cls.offr_cls_id
      ,offr_cls.offr_cls_desc_txt
--           
  FROM (SELECT *
           FROM offr
...
--offr_cls
 AND offr_cls.offr_cls_id = o.offr_cls_id (+)
   )
     LOOP
      PIPE ROW(obj_edit_offr_line(rec.status,
                                  rec.mrkt_id,
  ...
  BEGIN
    ...
    SELECT obj_edit_offr_line(
                ...
                pg_wght, sprd_nr, offr_prfl_prcpt_id, has_unit_qty, offr_typ, forcasted_units, forcasted_date, offr_cls_id, offr_cls_desc_txt)
      BULK COLLECT
    ...
    LOOP
      ...
      p_edit_offr_table(p_edit_offr_table.LAST)
        := obj_edit_offr_line(p_status, osl_rec.mrkt_id, osl_rec.offr_perd_id, osl_rec.offr_lock, osl_rec.offr_lock_user,
                              ...
                              osl_rec.forcasted_units, osl_rec.forcasted_date, osl_rec.offr_cls_id, osl_rec.offr_cls_desc_txt);
    END LOOP;
