...
FUNCTION get_offr(p_get_offr IN obj_get_offr_table)
    ...
    FOR rec IN (
                  ...
                  ,CASE
                     WHEN osl_latest.nr_for_qty = 0 THEN
                      0
                     ELSE
                      round(osl_latest.offr_prfl_sls_prc_amt / osl_latest.nr_for_qty * osl_latest.unit_qty * osl_latest.net_to_avon_fct,4)
                   END lv_gross_sales
                  ,CASE
                     WHEN osl_latest.nr_for_qty = 0 THEN
                      0
                     ELSE
                     round(osl_latest.offr_prfl_sls_prc_amt / osl_latest.nr_for_qty * osl_latest.unit_qty * osl_latest.net_to_avon_fct - osl_latest.sku_cost_amt * osl_latest.unit_qty,4)  
                   END lv_dp_cash
                  ,CASE
                     WHEN osl_latest.nr_for_qty = 0
                      ...
                      0
                     ELSE
                     round((osl_latest.offr_prfl_sls_prc_amt / osl_latest.nr_for_qty * osl_latest.unit_qty * osl_latest.net_to_avon_fct - osl_latest.sku_cost_amt * osl_latest.unit_qty) /
                          (osl_latest.offr_prfl_sls_prc_amt / osl_latest.nr_for_qty * osl_latest.unit_qty * osl_latest.net_to_avon_fct),4) * 100
                   END lv_dp_percent
                   --/latest version calculations
                  ,0 AS status
                  ...
                  ,CASE WHEN osl_current.offr_sku_line_id IS NOT NULL THEN
                  ...
                  ,COALESCE(
                   (SELECT MAX(spa) KEEP (dense_rank LAST ORDER BY skuid,sumu,vehid)
                    FROM
                    (SELECT 
                    osl_sp.sku_id AS skuid, SUM(dms_sp.unit_Qty) sumu, osl_sp.veh_id vehid,max(oppp.sls_prc_amt) KEEP(dense_rank LAST ORDER BY osl_sp.sku_id,osl_sp.veh_id) spa
                     FROM offr_prfl_prc_point oppp, offr_sku_line osl_sp, dstrbtd_mrkt_sls dms_sp, offr o_sp WHERE 
                                          oppp.offr_prfl_prcpt_id = osl_sp.offr_prfl_prcpt_id
                                      AND osl_sp.mrkt_id = l_mrkt_id
                                      AND osl_sp.offr_perd_id = (SELECT MAX(tpymp.prev_yr_perd_id) keep(dense_rank LAST ORDER BY tpymp.last_updt_ts) AS prev_yr_perd_id
                                                                  FROM trnsfrm_prev_yr_mrkt_perd tpymp
                                                                 WHERE tpymp.mrkt_id = l_mrkt_id
                                                                   AND tpymp.perd_id = l_offr_perd_id)
                                      AND dms_sp.offr_sku_line_id = osl_sp.offr_sku_line_id
                                      AND dms_sp.sls_typ_id IN (6,7)
                                      AND o_sp.ver_id = 0
                                      AND o_sp.offr_id = osl_sp.offr_id
                                      GROUP BY osl_sp.sku_id,osl_sp.veh_id,oppp.offr_prfl_prcpt_id)
                  ...
                  ,(NVL((SELECT MAX(1)
                           FROM offr_sku_line osl,
                                ...
                                sls_typ st
                          WHERE dms.offr_sku_line_id = osl.offr_sku_line_id
                            AND osl.offr_prfl_prcpt_id = offr_prfl_prc_point.offr_prfl_prcpt_id
                            AND st.sls_typ_id = dms.sls_typ_id
                            AND osl.mrkt_id = l_mrkt_id
                            AND osl.offr_perd_id = l_offr_perd_id
                            AND st.sls_typ_grp_nm = 'Actual'
                            AND dms.ver_id = 0
                            AND dms.unit_qty > 0), 0)) AS has_unit_qty
                  ,o.offr_typ
                    FROM (SELECT *
                             FROM offr
                            WHERE offr_id IN
                              (SELECT p_offr_id FROM table(p_get_offr)) AND
                        ...
                        , (SELECT offr_sku_line.offr_sku_line_id
                             ...
                             FROM offr_sku_line
                                 ...
                                 ,sku
                                 ,offr        o
                            WHERE --dms_current.offr_sku_line_id(+) =
                           --offr_sku_line.offr_sku_line_id
                           -- AND
                        ...
                        ,(SELECT osl_ltst.*
                          ...
                          --osl ltst subselect
                            FROM (SELECT osl.offr_sku_line_id
                                        ...
                                        ,dms.sls_typ_id
                                        ,SUM(dms.net_to_avon_fct *
                                             dms.unit_qty) /
                                         SUM(dms.unit_qty) AS net_to_avon_fct
                                    FROM offr_sku_line              osl
                                        ,offr                       o
                                        ,dstrbtd_mrkt_sls           dms
                                        ,latest_version_and_sls_typ
                                   WHERE o.offr_id = osl.offr_id
                                     AND osl.offr_sku_line_id =
                                         dms.offr_sku_line_id
                                     AND dms.ver_id =
                                     ...
                                     AND dms.unit_qty <> 0
                                     AND o.mrkt_id = l_mrkt_id
                                     AND dms.veh_id = latest_version_and_sls_typ.veh_id
  ...
  PROCEDURE add_concepts(p_offr_id      IN NUMBER,
    ...
    l_wghtd_avg_cost_amt     NUMBER; 
    l_sls_srce_id            NUMBER;
    l_tax_type_id            NUMBER;
  ...
  BEGIN
    ...
    l_pg_typ_id        := to_number(l_default_arr(2));
    l_ssnl_evnt_id     := to_number(l_default_arr(3));
    ...
    FOR i IN p_prfl_cd_list.FIRST .. p_prfl_cd_list.LAST LOOP
      ...
      FOR sku_rec IN (
        SELECT pa_maps_public.get_sls_cls_cd(p_offr_perd_id, p_mrkt_id, s.sku_id) sls_cls_cd,
               s.sku_id,
               srp.reg_prc_amt
          FROM sku s,
               sku_reg_prc srp
      ...
      LOOP
        BEGIN
          ...
          SELECT 1 INTO l_found
           ...
           WHERE offr_id        = p_offr_id
             ...
             AND prfl_cd        = p_prfl_cd_list(i)
             AND pg_ofs_nr      = l_pg_ofs_nr
             AND featrd_side_cd = l_concept_featrd_side_cd;
        EXCEPTION
          WHEN no_data_found THEN
          ...
          VALUES
            ( p_offr_id, sku_rec.sls_cls_cd, p_prfl_cd_list(i), l_pg_ofs_nr, l_concept_featrd_side_cd, p_mrkt_id, p_veh_id,
        ...
        BEGIN        
          ...
          SELECT offr_prfl_prcpt_id, crncy_cd, sls_prc_amt, net_to_avon_fct
            INTO l_offr_prfl_prcpt_id, l_crncy_cd, l_sls_prc_amt, l_net_to_avon_fct
            FROM offr_prfl_prc_point opp
           WHERE opp.offr_id        = p_offr_id
             AND opp.prfl_cd        = p_prfl_cd_list(i)
             AND opp.sls_cls_cd     = sku_rec.sls_cls_cd
             AND opp.sls_prc_amt    = l_sls_prc_amt
             AND opp.nr_for_qty     = l_nr_for_qty
             AND opp.pg_ofs_nr      = l_pg_ofs_nr
             AND opp.featrd_side_cd = l_concept_featrd_side_cd;
        EXCEPTION
          WHEN no_data_found THEN
            ...
            -- calculate commission, tax and GTA
            BEGIN
              l_tax_type_id := pa_maps_gta.get_default_tax_type_id(p_mrkt_id, p_prfl_cd_list(i), sku_rec.sls_cls_cd, p_offr_perd_id, p_veh_id);
            ...
            BEGIN
              l_comsn_typ := pa_maps_gta.get_commission_type(p_mrkt_id, p_veh_id, p_offr_perd_id, p_prfl_cd_list(i), 'N',
            ...
            VALUES
              ( l_offr_prfl_prcpt_id, p_offr_id, l_promtn_clm_id, p_veh_id, l_promtn_id, p_mrkt_id,
                sku_rec.sls_cls_cd, p_prfl_cd_list(i), l_ssnl_evnt_id, p_offr_perd_id, l_crncy_cd, 0, l_nr_for_qty,
        ...
        EXCEPTION
          WHEN no_data_found THEN
            ...
            VALUES
              ( p_offr_id, sku_rec.sls_cls_cd, p_prfl_cd_list(i), l_pg_ofs_nr, l_concept_featrd_side_cd,
  ...
  PROCEDURE add_offer(p_mrkt_id                IN NUMBER,
    ...
    l_featrd_side_cd         NUMBER;
    l_offr_stus_cd           NUMBER;
    l_offr_cls_id            NUMBER;
    l_offr_typ               VARCHAR2(3);
    l_flap_ind               VARCHAR2(1);
    l_brchr_postn_id         NUMBER;
    l_unit_rptg_lvl_id       NUMBER;
    l_rpt_sbtl_typ_id        NUMBER;
  ...
  BEGIN
    ...
    l_pg_typ_id        := to_number(l_default_arr(2));
    l_ssnl_evnt_id     := to_number(l_default_arr(3));
    l_sctn_page_ofs_nr := to_number(l_default_arr(4));
    l_featrd_side_cd   := to_number(l_default_arr(5));
    l_offr_stus_cd     := to_number(l_default_arr(6));
    l_offr_cls_id      := to_number(l_default_arr(7));
    l_offr_typ         := l_default_arr(8);
    l_flap_ind         := l_default_arr(9);
    l_brchr_postn_id   := to_number(l_default_arr(10));
